...
!--------------------------------------------------------------------
!	    zonal clouds radiative properties module
!       currently a wrapper until SKYHI goes away and this
!       module can be consolidated  with zonal_clouds_mod
!
!--------------------------------------------------------------------
...
public          &
	  zonal_clouds_init, zonal_clouds_calc
...
!-----  read namelist  ------
      ...
      if (file_exist('input.nml')) then
        ...
        read (unit, nml=zonal_clouds_W_nml, iostat=io, end=10)
        ierr = check_nml_error (io, 'zonal_clouds_W_nml')
        enddo
10      call close_file (unit)
      endif

      unit = open_file ('logfile.out', action='append')
!     call print_version_number (unit, 'zonal_clouds_W', version_number)
      if (get_my_pe() == 0) then
	write (unit,'(/,80("="),/(a))') trim(version), trim(tag)
	write (unit,nml=zonal_clouds_W_nml)
      endif
      call close_file (unit)

!--------------------------------------------------------------------
!  define cloud reflectivities for high, middle and low clouds depend-
!  ent on the desired peripherals set
!--------------------------------------------------------------------
...
end subroutine zonal_clouds_init
...
!------------------------------------------------------------------
!    call getcloud to obtain cloud amounts and locations.
!------------------------------------------------------------------
...
end subroutine zonal_clouds_calc


!####################################################################


	       end module zonal_clouds_W_mod
