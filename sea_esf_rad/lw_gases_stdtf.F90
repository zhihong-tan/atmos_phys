...
!---------------------------------------------------------------------
!          module which computes line-by-line transmission 
...
!   call print_version_number (unit, 'lw_gases_stdtf', version_number)
    if (get_my_pe() == 0) then
      write (unit,'(/,80("="),/(a))') trim(version), trim(tag)
      write (unit,nml=lw_gases_stdtf_nml)
    endif
...
!--------------------------------------------------------------------
        call gasins('n2o',                                        &
                    do_lvlcalc_n2o, do_lvlctscalc_n2o, do_lyrcalc_n2o, &
                    nf, ntbnd_n2o(nf),                            &
                    ndimkp,ndimk,                               &
                    dgasdt10_lvl, dgasdt10_lvlcts, dgasdt10_lyr,   &
                    gasp10_lvl, gasp10_lvlcts, gasp10_lyr,      &
...
!---------------------------------------------------------------------
	...
	unit = open_file ('INPUT/stdlvls', form= 'formatted',  &
			  action='read')
	do k=1,496
...
!  overall ~ 10% speedup in standalone code -- no change in radiag file
            ...
            do kp=1,k1
              ...
              d2gast10_lvl(kp,k) = .0016*    &
					...
					trns_interp_lvl_ps_nf(kp,k,1))*&
                                 1000.
              d2gast8_lvl(kp,k)  =   &
				.0016*(trns_interp_lvl_ps8_nf(kp,k,2) +&
				      trns_interp_lvl_ps8_nf(kp,k,3) - &
                                 2.0* &
				 trns_interp_lvl_ps_nf(kp,k,1))*  &
                                 1000.
          ...
          do k=1,k2
            do kp=1,k1
              ...
              d2gast8_lyr(kp,k)  = .0016*  &
				  ...
				  trns_interp_lyr_ps8_nf(kp,k,3) - &
                                 2.0*trns_interp_lyr_ps8_nf(kp,k,1))*  &
                                 1000.
            enddo
          enddo
        endif
...
!-------------------------------------------------------------------
!   there is no need to call intcoef_1d, as no interpolation from
!   the (ca,xa, sexp, uexp) values actually occurs.
!-------------------------------------------------------------------
!     call intcoef_1d (pa, ca, sexp, xa, uexp)
...
!    write (stdout, 360)  k, check
	    write (     *, 360)  k, check
360         format ('check le zero, i=',i3, ' check =',f20.10)
            call error_mesg ('coeint',  &
...
!    write (stdout, 361) k, upath0(k), xa(k) 
...
!---------------------------------------------------------------------
 	    errorint(kp,k) = 0.0
!           errorint(kp,k) =     &
!              (pressint_hiv(kp,k) - pressint_lov(kp,k))*    &
!               error(indx_press_hiv(kp,k)+1,indx_press_hiv(kp,k))/    &
!              (pa(indx_press_hiv(kp,k)+1) - pa(indx_press_hiv(kp,k)))
          endif
	enddo
      enddo
