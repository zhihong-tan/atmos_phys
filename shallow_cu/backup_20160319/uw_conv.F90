...
!miz
  ...
  NAMELIST / uw_conv_nml / iclosure, rkm_sh1, rkm_sh, cldhgt_max, plev_cin, negbuo_max, do_peff_land, &
...
!------------------------------------------------------------------------
  ...
  integer :: id_tdt_uwc, id_qdt_uwc, id_udt_uwc, id_vdt_uwc, id_prec_uwc, id_snow_uwc, &
       ...
       id_cfq_uwc, id_fdp_uwc, id_fsh_uwc, id_hmo_uwc, id_hms_uwc, id_abu_uwc, id_peo_uwc, &
...
!<--- h1g, 2015-08-11
    ...
    id_negbuo_uws = register_diag_field (mod_name,'negbuo_uws', axes(1:2), Time, &
...
!========Option for deep convection=======================================
    if (do_deep) then
       ...
       id_negbuo_uwd = register_diag_field (mod_name,'negbuo_uwd', axes(1:2), Time, &
            'negative buoyancy for penetrative plume', 'K', interp_method = "conserve_order1" )
       if ( do_strat ) then
...
!CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
    ...
    real, dimension(size(tb,1),size(tb,2)) :: &
	 ...
	 tdt_rad_fre, tdt_dyn_fre, tdt_dif_fre, qdt_dyn_fre, qdt_dif_fre, &
         tdt_tot_pbl, tdt_tot_fre, cpool, bflux,dgz_dyn_int, ddp_dyn_int, lts, negbuo_s, negbuo_d
...
!========Option for deep convection=======================================
   ...
   !initialize 3D variables outside the loop
    ...
    hmint_old=hmint; hmint=0; lts=0.; negbuo_s=0.; negbuo_d=0.; lofactor=1.;
...
!     end where
    ...
    do i=1,imax
       do j=1,jmax
       	  if (cush(i,j)   .gt. 0) fsh(i,j)=1.;
	  if (cush_d(i,j) .gt. 0) fdp(i,j)=1.;
       	  do k=1,kmax
             cfq(i,j,k) = 0
             if (cmf(i,j,k) .gt. 0.) then
             	cfq(i,j,k) = 1
             endif
          enddo
       enddo
...
!      used = send_data( id_tten_rad_uwc,tten_rad*aday,Time, is, js, 1)
    ...
    used = send_data( id_negbuo_uws,   negbuo_s, Time, is, js )
...
!       used=send_data( id_trwet_uwd,  trwet,         Time, is, js, 1)
       ...
       used=send_data( id_negbuo_uwd,negbuo_d,             Time, is, js )
