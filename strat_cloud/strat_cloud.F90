...
use lscloud_netcdf_mod,        only : lscloud_netcdf
use aerosol_cloud_mod,         only : aerosol_effects
...
!
!       strat_cloud_end        module destructor
!

public  strat_cloud_init, strat_cloud, strat_cloud_end

private strat_cloud_legacy, ppm2m_sak, kmppm_sak, steepz_sak,   &
        cloud_clear_xfer
...
namelist / strat_cloud_nml /   &
            U00, u00_profile, rthresh, use_kk_auto, U_evap, U_evap_snow,  &
            eros_scale, eros_choice, eros_scale_c, eros_scale_t, &
            mc_thresh, diff_thresh, super_choice, tracer_advec, Dmin, &
            efact, vfact, cfact, do_old_snowmelt, iwc_crit, vfall_const2, &
            vfall_exp2, num_mass_ratio1, num_mass_ratio2,   &
            retain_cm3_bug, var_limit,                       & 
...
!  </OVERVIEW>
!  <DESCRIPTION>
!       Initializes strat_cloud.  Reads namelist, initializes netcdf 
!       output.
!  </DESCRIPTION>
!  <TEMPLATE>
...
!
!------------------------------------------------------------------------
subroutine strat_cloud_init (Nml_mp, Nml_lsc, Exch_ctrl, Physics_control)
...
!----------------------------------------------------------------------
!    retrieve and store module variables obtained from other modules.
!----------------------------------------------------------------------
...
!------------------------------------------------------------------------
      if (do_liq_num) then     
        call strat_cloud_legacy  (     &
           ...
           Cloud_state%ql_in, Cloud_state%qi_in, Cloud_state%qa_in,   &
           Input_mp%omega, C2ls_mp%mc_full, Input_mp%diff_t,   &
           Input_mp%LAND, Tend_mp%ttnd, Tend_mp%qtnd,   &
           Tend_mp%q_tnd(:,:,:,nql), Tend_mp%q_tnd(:,:,:,nqi), &
           Tend_mp%q_tnd(:,:,:,nqa), Cloud_processes%f_snow_berg,   &
           Removal_mp%rain3d, Removal_mp%snow3d, Removal_mp%snowclr3d, &
           Precip_state%surfrain, Precip_state%surfsnow,&
           C2ls_mp%convective_humidity_ratio,    &
           C2ls_mp%convective_humidity_area,&
           limit_conv_cloud_frac, qn= Cloud_state%qn_in, &
           Aerosol=Aerosol, SN=Tend_mp%q_tnd(:,:,:,nqn))
      else
        call strat_cloud_legacy  (     &
           Particles%concen_dust_sub, Particles%totalmass1, &
           Atmos_state%pthickness, cloud_generator_on, &
...
subroutine strat_cloud_legacy (       &
          concen_dust_sub, totalmass1, pthickness, cloud_generator_on,   &
          diag_id, diag_pt, n_diag_4d, n_diag_4d_kp1, diag_4d,    &
          diag_4d_kp1, diag_3d, Time, is, ie, js, je, dtcloud, pfull,  &
          phalf, radturbten2, T, qv, ql, qi, qa, omega, Mc, diff_t, LAND, &
          ST, SQ, SL, SI, SA, f_snow_berg, rain3d, snow3d, snowclr3d,  &
          surfrain, surfsnow, qrat, ahuco, limit_conv_cloud_frac,      &
          qn, Aerosol, SN)
...
!
      ...
      real, dimension(:,:,:),     intent(inout) :: concen_dust_sub
      real, dimension(:,:,:,:),   intent(inout) :: totalmass1        
      ...
      real, dimension(:,:,:), intent (in)     :: pfull,phalf, pthickness
      real, dimension(:,:,:), intent (in)     :: T,qv,ql,qi,qa,omega
      real, dimension(:,:,:), intent (in)     :: Mc, diff_t
      real, dimension(:,:,:), intent (in)     :: qrat,ahuco
      real, dimension(:,:),   intent (in)     :: LAND
      real, dimension(:,:,:), intent (in)     :: radturbten2
      real, dimension(:,:,:), intent (out)    :: ST,SQ,SL,SI,SA
      real, dimension(:,:),   intent (out)    :: surfrain,surfsnow
      real, dimension(:,:,:), intent (out)    :: f_snow_berg
      real, dimension(:,:,:), intent (out)    :: rain3d
      real, dimension(:,:,:), intent (out)    :: snow3d
      real, dimension(:,:,:), intent (out)    :: snowclr3d

      real, dimension(:,:,:), intent(in),   optional :: qn
      type(aerosol_type),     intent (in),  optional :: Aerosol  
...
!
        ...
        real, dimension(size(T,1),size(T,2),size(T,3),12) :: imass1
        ...
        real, dimension(size(T,1),size(T,2))   :: tmp1,tmp2,tmp3,tmp5,  &
                                                  delta_cf,drop1,crystal
...
!-----------------------------------------------------------------------
!   save out the liquid water as a weighting factor to be used with
!   some diagnostics.   
!-----------------------------------------------------------------------
...
!---------------------------------------------------------------------
        if (do_liq_num .or. do_dust_berg) then
          call aerosol_effects ( idim, jdim, kdim, n_diag_4d, pthickness, &
...
!
        ...
        if (nsublevels.gt.1) then
          do id = 1,idim
            call ppm2m_sak(qta4(:,id,:,:),delp(id,:,:),  &
                                                 kdim,kmap,1,jdim,0,kord)
            call ppm2m_sak(qtqsa4(:,id,:,:),delp(id,:,:), &
                                                 kdim,kmap,1,jdim,0,kord)
          enddo                
        else
          qta4(2,:,:,:) = qta4(1,:,:,:)
     ...
     if (.not. do_pdf_clouds) then
      ...
      if (max(diag_id%qidt_fill,diag_id%qi_fill_col) > 0) then
        ...
        if (.not. do_liq_num) then
          ...
          if (diag_id%cf_liq_init   > 0)  &
                      diag_4d(:,:,:,diag_pt%cf_liq_init  ) = min(qa,1.)
          do j=1,kdim
           do k=1,jdim
            do i=1,idim
             if (ql(i,k,j).le.qmin .or. qa(i,k,j).le.qmin .or.  &
              ...
              if (max(diag_id%qldt_fill,diag_id%ql_fill_col) > 0)  &
                diag_4d(i,k,j,diag_pt%qldt_fill) = -ql(i,k,j) * inv_dtcloud
              ...
              if (diag_id%cf_liq_init   > 0)   &
                                 diag_4d(i,k,j,diag_pt%cf_liq_init  ) = 0.
             endif
            enddo
           enddo
          enddo
...
!       inside and outside of the cloud are about the same.
          if (qa_upd(i,k) .gt. U(i,k)) then
             if (max(diag_id%qadt_rhred,diag_id%qa_rhred_col) > 0 )  &
                     diag_4d(i,k,j,diag_pt%qadt_rhred) =   &
                                      (qa_upd(i,k)-U(i,k)) * inv_dtcloud
...
!ljd
        ...
        if (dqs_ls(i,k).le.0. .and. U(i,k).ge.U00p(i,k) .and.   &
             ...
             tmp1s = sqrt( (1.+qa_upd(i,k)*gamma(i,k,j))**2. -   &
                                                  (1.-qa_upd(i,k)) * &
                    (1.-qa_upd(i,k))*gamma(i,k,j)*dqs_ls(i,k)/qs(i,k,j)/  &
                    max(1.-U(i,k),qmin) ) - (1.+qa_upd(i,k)*gamma(i,k,j))
             tmp1s = -1. * tmp1s / ((1.-qa_upd(i,k))*   &
                                         (1.-qa_upd(i,k))*gamma(i,k,j)/&
                    qs(i,k,j)/max(1.-U(i,k),qmin)/2.)
             dqs_ls(i,k) = min(tmp1s,dqs_ls(i,k)/   &
                                  (1.+0.5*(1.+qa_upd(i,k))*gamma(i,k,j)))
        else
...
!       end where 
        ...
        if ((dqs_ls(i,k).le.0. .and. U(i,k).ge.U00p(i,k)) .and. &
             ...
             da_ls(i,k) = -0.5 * (1.-qa_upd(i,k)-ahuco(i,k,j)) *   &
                                  (1.-qa_upd(i,k)- ahuco(i,k,j)) *  &
                            dqs_ls(i,k)/   qs(i,k,j) / max(1.-U(i,k),qmin)
        endif
      enddo
     enddo
...
!        taken by convective clouds
        ...
        if (max(diag_id%qadt_lsform,diag_id%qa_lsform_col) > 0)   &
                   diag_4d(i,k,j,diag_pt%qadt_lsform) =    &
                                        C_dts * (1.-qcbars) * inv_dtcloud 
        if (max(diag_id%qadt_eros,diag_id%qa_eros_col)  > 0)   &
                   diag_4d(i,k,j,diag_pt%qadt_eros) =     &
                                         D_dts *     qcbars  * inv_dtcloud
...
!rab        qc1 = qag
        ...
        if (max(diag_id%qadt_lsform,diag_id%qa_lsform_col) > 0)   &
                   diag_4d(:,:,j,diag_pt%qadt_lsform) =    &
                                     max(qag-qa(:,:,j),0.) * inv_dtcloud 
        if (max(diag_id%qadt_lsdiss,diag_id%qa_lsdiss_col) > 0)   &
                   diag_4d(:,:,j,diag_pt%qadt_lsdiss) =    &
                                      max(qa(:,:,j)-qag,0.) * inv_dtcloud
          ...
          do k = 1,jdim
            do i = 1,idim
              if ( da_ls(i,k) > 0.0 ) then
                ...
                if (diag_id%subgrid_w_variance > 0 .and. up_strat > 0.0) &
                          diag_4d(i,k,j,diag_pt%subgrid_w_variance) = wp2
                call aer_ccn_act_wpdf (T(i,k,j), pfull(i,k,j), &
                                       up_strat, wp2,    &
                                       totalmass1(i,k,j,:), drop1(i,k))
                if (diag_id%potential_droplets > 0 .and. up_strat > 0.0) &
                     diag_4d(i,k,j,diag_pt%potential_droplets) = drop1(i,k)
                ...
                if (diag_id%subgrid_w_variance > 0)   &
                          diag_4d(i,k,j,diag_pt%subgrid_w_variance) =  0.0
                if (diag_id%potential_droplets > 0)   &
                           diag_4d(i,k,j,diag_pt%potential_droplets) = 0.0        
              endif
            end do
          end do        
        endif
...
!
        ...
        call cloud_clear_xfer (cloud_generator_on, tmp3, qa_mean,   &
                  qa_mean_lst, a_rain_clr, a_rain_cld, rain_clr, rain_cld)
!
...
!
        ...
        call cloud_clear_xfer (cloud_generator_on, tmp3, qa_mean,   &
                   qa_mean_lst, a_snow_clr, a_snow_cld, snow_clr, snow_cld)
...
!       tfreeze.
     ...
     do k=1,jdim
      do i=1,idim
        ...
        tmp1s = cp_air*(T(i,k,j)-tfreeze-snow_fact)*  &
                                           deltpg(i,k)*inv_dtcloud/hlf
        ...
        if (max(diag_id%snow_melt,diag_id%snow_melt_col) > 0)   &
                     diag_4d(i,k,j,diag_pt%snow_melt) = tmp2s/deltpg(i,k)             
...
!
        ...
        if (max(diag_id%snow_melt,diag_id%snow_melt_col) > 0)   &
                 diag_4d(i,k,j,diag_pt%snow_melt) =   &
                       diag_4d(i,k,j,diag_pt%snow_melt) + tmp2s/deltpg(i,k)
...
!       into the grid box in the same time step.
        ...
        if (max(diag_id%qidt_fall,diag_id%qi_fall_col) > 0)   &
                   diag_4d(:,:,j,diag_pt%qidt_fall)= -1.*snow_cld/deltpg
...
!       integration.  D1_dt represents the conversion of liquid to rain.
        ...
        if (diag_id%rvolume > 0)   &
                  diag_4d(:,:,j,diag_pt%rvolume) =   &
                                     rad_liq*diag_4d(:,:,j,diag_pt%aliq)
        ...
        if (max(diag_id%qldt_accr,diag_id%ql_accr_col)  > 0)   &
                               diag_4d(:,:,j,diag_pt%qldt_accr) = D1_dt
...
!********************yim's version based on Khairoutdinov and Kogan (2000)
          tmp1 = dtcloud * 1350. *  &
        (1.e-6*max(qn_mean*airdens(:,:,j),max(qa_mean,qmin)*N_min))**  &
                                                                (-1.79)*  &
...
!yim fall back to M & C using qn
             ...
             tmp1 = 32681. * dtcloud *   &
                     ((max(qn_mean,qmin)*airdens(:,:,j)*dens_h2o)**  &
                                                        (-1./3.))*       &
        ...
        if (max(diag_id%qldt_auto,diag_id%ql_auto_col) > 0)   &
                             diag_4d(:,:,j,diag_pt%qldt_auto) = tmp1     

        if ( diag_id%aauto > 0 ) then
             where ( rad_liq .gt. rthresh )   &
                            diag_4d(:,:,j,diag_pt%aauto) = qa_mean       
        end if
...
!       which have much lower densities.
        ...
        do k = 1,jdim
          do i = 1,idim
           if ( (T(i,k,j) .lt. tfreeze) .and. (ql_mean(i,k) .gt. qmin)   &
             ...
             call Jhete_dep(T(i,k,j),Si0,concen_dust_sub(i,k,j),  &
                                                           crystal(i,k))
         ...
         if (max(diag_id%qndt_cond,diag_id%qn_cond_col) > 0)   &
                           diag_4d(:,:,j, diag_pt%qndt_cond) = crystal
         if (max(diag_id%qndt_evap,diag_id%qn_evap_col) > 0) then
           diag_4d(:,:,j,diag_pt%qndt_evap) = 0.
           where (T(:,:,j).lt.tfreeze .and. ql_mean.gt.qmin .and.   &
                                                        qa_mean.gt.qmin)  
            diag_4d(:,:,j,diag_pt%qndt_evap) =    &
                      1.e-3*exp((12.96*0.0125*(tfreeze-T(:,:,j)))-0.639)
           end where
         endif
       ...
       where (T(:,:,j) .lt. tfreeze .and. ql_mean .gt. qmin .and.   &
                                                       qa_mean .gt. qmin) 
         D2_dt =  dtcloud*qa_mean*((1.e6*crystal(:,:)/airdens(:,:,j))**  &
                                                   (2./3.))* 7.8*   &
               ((max(qi_mean/qa_mean,1.E-12*1.e6*crystal(:,:)     &
        ...
        sum_berg = D2_dt
        if (max(diag_id%qldt_berg,diag_id%ql_berg_col) > 0)   &
                               diag_4d(:,:,j,diag_pt%qldt_berg) = D2_dt
...
!       prevented when temperatures were in excess of freezing.
        ...
        sum_rime = tmp1
        if (max(diag_id%qldt_rime,diag_id%ql_rime_col) > 0)   &
                                 diag_4d(:,:,j,diag_pt%qldt_rime) = tmp1
...
!
        ...
        if (max(diag_id%qldt_freez,diag_id%qldt_rime,  &
                         diag_id%qldt_berg,diag_id%ql_freez_col,  &
                         diag_id%ql_rime_col,diag_id%ql_berg_col) > 0) then
          do k=1,jdim
           do i=1,idim
             if (T(i,k,j).lt.(tfreeze-40.).and.(ql_mean(i,k).gt.qmin)    &
               .and.(qa_mean(i,k).gt.qmin)) then
               if (max(diag_id%qldt_freez,diag_id%ql_freez_col) > 0) &
                         diag_4d(i,k,j,diag_pt%qldt_freez) = D2_dt(i,k)
               if (max(diag_id%qldt_rime,diag_id%ql_rime_col) > 0)  &
                         diag_4d(i,k,j,diag_pt%qldt_rime ) = 0.
               if (max(diag_id%qldt_berg,diag_id%ql_berg_col) > 0) &
                           diag_4d(i,k,j,diag_pt%qldt_berg ) = 0.     
             endif
           enddo
          enddo
        end if
...
!rab        tmp2 = -Dterm/max(D_dt,Dmin)
   ...
   if (retain_cm3_bug) then
        ...
        ST(i,k,j) = ST(i,k,j) + (hlv*max(dcond_ls(i,k),0.)/cp_air) -    &
             (hlv*(max(-1.*dcond_ls(i,k),0.) /max(ql_mean(i,k),qmin))*  &
                                                         tmp2(i,k)/cp_air)
        ...
        ST(i,k,j) = ST(i,k,j) + (hlf*D2_dt(i,k)-hlv*D_eros(i,k))*   &
                                                         tmp2(i,k)/cp_air
        ...
        rain_cld(i,k) = rain_cld(i,k) +D1_dt(i,k)*tmp2(i,k)*  &
                                                 deltpg(i,k)*inv_dtcloud
...
!        end where
         ...
         if (max(diag_id%qndt_evap,diag_id%qn_evap_col) > 0)   &
                  diag_4d(i,k,j,diag_pt%qndt_evap) = D_dts * qcbars !Dterm
            end do
          end do
...
!       
        ...
        if (max(diag_id%qldt_cond,diag_id%ql_cond_col) > 0)  &
         diag_4d(:,:,j,diag_pt%qldt_cond)  =   &
                                            max(dcond_ls,0.) *inv_dtcloud
        if (max(diag_id%qldt_evap,diag_id%ql_evap_col) > 0)   &
               diag_4d(:,:,j,diag_pt%qldt_evap)  =  &
               (max(0.,-1.*dcond_ls )/max(ql_mean,qmin))*tmp2*inv_dtcloud
        if (max(diag_id%qldt_accr,diag_id%ql_accr_col) > 0)   &
                diag_4d(:,:,j,diag_pt%qldt_accr)  =   &
                       diag_4d(:,:,j,diag_pt%qldt_accr)*tmp2*inv_dtcloud
        if (max(diag_id%qldt_auto,diag_id%ql_auto_col) > 0)   &
                 diag_4d(:,:,j,diag_pt%qldt_auto)  =   &
                       diag_4d(:,:,j,diag_pt%qldt_auto )*tmp2*inv_dtcloud
        if (max(diag_id%qldt_berg,diag_id%ql_berg_col) > 0)    &
                  diag_4d(:,:,j,diag_pt%qldt_berg)  =   &
                       diag_4d(:,:,j,diag_pt%qldt_berg )*tmp2*inv_dtcloud
        if (max(diag_id%qldt_rime,diag_id%ql_rime_col) > 0)   &
                  diag_4d(:,:,j,diag_pt%qldt_rime)  =    &
                       diag_4d(:,:,j,diag_pt%qldt_rime )*tmp2*inv_dtcloud
        if (max(diag_id%qldt_freez,diag_id%ql_freez_col) > 0)   &
                  diag_4d(:,:,j,diag_pt%qldt_freez) =    &
                       diag_4d(:,:,j,diag_pt%qldt_freez)*tmp2*inv_dtcloud
        if (max(diag_id%qndt_cond,diag_id%qn_cond_col) > 0)   &
                  diag_4d(:,:,j,diag_pt%qndt_cond)  =   &
                             diag_4d(:,:,j,diag_pt%qndt_cond)*inv_dtcloud 
        if (max(diag_id%qndt_evap,diag_id%qn_evap_col) > 0)   &
                  diag_4d(:,:,j,diag_pt%qndt_evap)  =    &
                              diag_4d(:,:,j,diag_pt%qndt_evap)*inv_dtcloud 

        if (max(diag_id%qldt_eros,diag_id%ql_eros_col) > 0)   &
                   diag_4d(:,:,j,diag_pt%qldt_eros)  =    &
                                D_eros*tmp2*inv_dtcloud 
...
!       implicitness to the scheme.
        ...
        if (diag_id%vfall > 0)    &
           diag_4d(:,:,j,diag_pt%vfall) =   &
                                  Vfall(:,:)*diag_4d(:,:,j,diag_pt%aice)
...
!rab        tmp2 = -Dterm/max(D_dt,Dmin)
   if (retain_cm3_bug) then
        ...
        ST(i,k,j) = ST(i,k,j) +  hls*max(dcond_ls_ice(i,k),0.)/cp_air -  &
         hls*(max(-1.*dcond_ls_ice(i,k),0.)/max(qi_mean(i,k),qmin))*  &
                                                             tmp2s/cp_air
        ...
        rain_cld(i,k) = rain_cld(i,k) + D2_dt(i,k)*tmp2s*deltpg(i,k)*   &
                                                              inv_dtcloud
...
!       
        ...
        if (max(diag_id%qidt_dep,diag_id%qi_dep_col) > 0) &
                diag_4d(i,k,j,diag_pt%qidt_dep ) =   &
                                   max(dcond_ls_ice(i,k),0.)*inv_dtcloud
        if (max(diag_id%qidt_subl,diag_id%qi_subl_col) > 0) &
                diag_4d(i,k,j,diag_pt%qidt_subl) =   &
              (max(0.,-1.*dcond_ls_ice(i,k))/max(qi_mean(i,k),qmin))*   &
                                                        tmp2s*inv_dtcloud
        if (max(diag_id%qidt_melt,diag_id%qi_melt_col) > 0) &
                diag_4d(i,k,j,diag_pt%qidt_melt) =    &
                                          D2_dt(i,k) *tmp2s*inv_dtcloud
        if (max(diag_id%qidt_eros,diag_id%qi_eros_col) > 0) &
                diag_4d(i,k,j,diag_pt%qidt_eros) =   &
                                         D_eros(i,k)*tmp2s*inv_dtcloud   

        if (max(diag_id%lsf_strat,diag_id%lcf_strat,   &
                                         diag_id%mfls_strat) > 0) then
            tmp1s = max(dcond_ls_ice(i,k),0.)*inv_dtcloud
            if ( (diag_4d(i,k,j,diag_pt%qldt_cond) + tmp1s) .gt. 0. )   &
                                    diag_4d(i,k,j, diag_pt%lsf_strat) = 1.
        endif
       enddo
      enddo
...
!       ibution. 
      ...
      do k=1,jdim
       do i=1,idim
        ...
        !compute K3
        tmp1s = 56788.636 * dtcloud *    &
                           ((rain_clr(i,k)/max(a_rain_clr(i,k),qmin)/  &
             dens_h2o)**(11./18.))/SQRT(airdens(i,k,j))/A_plus_B(i,k,j)&
             /qs(i,k,j)

        !compute local change in vapor mixing ratio due to 
        !rain evaporation
        tmp1s = tmp1s*qs(i,k,j)*(1.-U_clr(i,k))/  &
                                      (1.+0.5*tmp1s*(1.+gamma(i,k,j)))

        !limit change in qv to the amount that would raise the relative
        !humidity to U_evap in the clear portion of the grid box
        tmp1s = min(tmp1s,((1.-qa_mean(i,k))/    &
                              max(a_rain_clr(i,k),qmin))*qs(i,k,j)* &
                              max(0.,U_evap-U_clr(i,k))/   &
                                        (1.+(U_evap*(1.-qa_mean(i,k))+  &
                                            qa_mean(i,k))*gamma(i,k,j)) )
        ...
        if (max(diag_id%rain_evap,diag_id%rain_evap_col) > 0)    &
                     diag_4d(i,k,j,diag_pt%rain_evap) = tmp2s/deltpg(i,k)
...
!rab       do i=1,idim
        ...
        !humidity to U_evap in the clear portion of the grid box
        tmp1s = min(tmp1s,((1.-qa_mean(i,k))/    &
                             max(a_snow_clr(i,k),qmin))*qs(i,k,j)* &
                             max(0.,U_evap_snow-U_clr(i,k))/    &
                          (1.+(U_evap_snow*(1.-qa_mean(i,k))+qa_mean(i,k))* &
                                                           gamma(i,k,j)) )
        ...
        if (max(diag_id%qdt_snow_sublim,diag_id%q_snow_sublim_col) > 0)   &
                diag_4d(i,k,j,diag_pt%qdt_snow_sublim) = tmp2s/deltpg(i,k)
       enddo 
      enddo 
...
!       desctruction code. (cjg)
        ...
        if (max(diag_id%qadt_destr,diag_id%qa_destr_col) > 0)   &
             diag_4d(:,:,j,diag_pt%qadt_destr) =    &
                 diag_4d(:,:,j,diag_pt%qadt_destr) + SA(:,:,j)*inv_dtcloud
        if (max(diag_id%qldt_destr,diag_id%ql_destr_col) > 0)   &
             diag_4d(:,:,j,diag_pt%qldt_destr) =     &
                 diag_4d(:,:,j,diag_pt%qldt_destr) + SL(:,:,j)*inv_dtcloud
        if (max(diag_id%qidt_destr,diag_id%qi_destr_col) > 0)   &
            diag_4d(:,:,j,diag_pt%qidt_destr) =   &
                 diag_4d(:,:,j,diag_pt%qidt_destr) + SI(:,:,j)*inv_dtcloud
IF (DO_LIQ_NUM) then
        if (max(diag_id%qndt_destr,diag_id%qn_destr_col) > 0)   &
            diag_4d(:,:,j,diag_pt%qndt_destr) =   &
                 diag_4d(:,:,j,diag_pt%qndt_destr) + SN(:,:,j)*inv_dtcloud
ENDIF
          ...
          do k=1,jdim
           do i=1,idim
            if ((ql_upd(i,k) .le. qmin .and. qi_upd(i,k) .le. qmin)      &
             ...
             ST(i,k,j) = ST(i,k,j) - (hlv*ql_upd(i,k) +    &
                                                  hls*qi_upd(i,k))/cp_air
...
!rab - save off tmp1 for diagnostic ice/liq adjustment fields in liq_adj array
        if (max(diag_id%ice_adj,diag_id%ice_adj_col,  &
                           diag_id%liq_adj,diag_id%liq_adj_col) .gt. 0) &
        ...
        if (super_choice) then
             ...
             if (max(diag_id%qadt_super,diag_id%qa_super_col) > 0) then
               where (tmp1 .gt. 0.)
                 diag_4d(:,:,j,diag_pt%qadt_super)  =    &
                                                (1.-qa_upd) * inv_dtcloud
               endwhere
             endif
                 ...
                 do i=1,idim
                   if (T(i,k,j) > tfreeze - 40. .and. &
                     ...
                     if (max(diag_id%qndt_super,diag_id%qn_super_col) > 0)  &
                        diag_4d(i,k,j,diag_pt%qndt_super) =   &
                              diag_4d(i,k,j,diag_pt%qndt_super) +   &
                                                drop1(i,k)*1.e6 / &
              ...
              do i=1,idim
               if(tmp1(i,k).gt.0) then
                if (T(i,k,j) .le. tfreeze-20.) then
                  snow_cld(i,k) = snow_cld(i,k) + qa_mean(i,k) *   &
                                        tmp1(i,k)*deltpg(i,k)*inv_dtcloud
                  snow_clr(i,k) = snow_clr(i,k) +   &
                            (1.-qa_mean(i,k))*tmp1(i,k)*deltpg(i,k)*  &
                                                             inv_dtcloud
                  a_snow_cld(i,k) = qa_mean(i,k)
                  a_snow_clr(i,k) = 1.-qa_mean(i,k)
                  ST(i,k,j)  = ST(i,k,j) + hls*tmp1(i,k)/cp_air
                else   ! where (T(i,k,j) .gt. tfreeze-20.)
                  rain_cld(i,k) = rain_cld(i,k) + qa_mean(i,k) *   &
                                        tmp1(i,k)*deltpg(i,k)*inv_dtcloud
                  rain_clr(i,k) = rain_clr(i,k) +    &
                               (1.-qa_mean(i,k))*tmp1(i,k)*deltpg(i,k)*  &
                                                             inv_dtcloud
...
!       add the ice falling out from cloud to qidt_fall
        ...
        if (max(diag_id%qidt_fall,diag_id%qi_fall_col) > 0)   &
            diag_4d(:,:,j,diag_pt%qidt_fall) =    &
                   diag_4d(:,:,j,diag_pt%qidt_fall) + (snow_cld/deltpg)
...
!       Save rain and snow diagnostics
        ...
        if (diag_id%rain_clr   > 0)   &
              diag_4d_kp1(:,:,j+1,diag_pt%rain_clr)     = rain_clr
        if (diag_id%rain_cld   > 0)  &
              diag_4d_kp1(:,:,j+1,diag_pt%rain_cld)     = rain_cld
        if (diag_id%a_rain_clr > 0)   &
              diag_4d_kp1(:,:,j+1,diag_pt%a_rain_clr)   = a_rain_clr
        if (diag_id%a_rain_cld > 0)   &
              diag_4d_kp1(:,:,j+1,diag_pt%a_rain_cld)   = a_rain_cld
        if (diag_id%snow_clr   > 0)   &
              diag_4d_kp1(:,:,j+1,diag_pt%snow_clr)     = snow_clr
        if (diag_id%snow_cld   > 0)   &
              diag_4d_kp1(:,:,j+1,diag_pt%snow_cld)     = snow_cld
        if (diag_id%a_snow_clr > 0)   &
              diag_4d_kp1(:,:,j+1,diag_pt%a_snow_clr)   = a_snow_clr
        if (diag_id%a_snow_cld > 0)   &
              diag_4d_kp1(:,:,j+1,diag_pt%a_snow_cld)   = a_snow_cld
        if (diag_id%a_precip_clr > 0)   &
              diag_4d_kp1(:,:,j+1,diag_pt%a_precip_clr) =   &
                                             max(a_rain_clr,a_snow_clr)
        if (diag_id%a_precip_cld > 0)   &
              diag_4d_kp1(:,:,j+1,diag_pt%a_precip_cld) =    &
...
!       DIAGNOSTICS
!
        if (max(diag_id%ice_adj,diag_id%ice_adj_col,diag_id%liq_adj, &
                                         diag_id%liq_adj_col) .gt. 0) then
          freeze_pt=40.
        ...
        if ( diag_id%subgrid_w_variance > 0 ) then
          diag_4d(:,:,:,diag_pt%subgrid_w_variance) =    &
                           diag_4d(:,:,:,diag_pt%subgrid_w_variance)**0.5
        endif

      
        if ( max(diag_id%lcf_strat,diag_id%mfls_strat) > 0 ) then
          where (omega(:,:,:)+grav*Mc(:,:,:) .lt. 0 .and.   &
                                  diag_4d(:,:,:,diag_pt%lsf_strat) .eq.1)
             diag_4d(:,:,:,diag_pt%lcf_strat) = 1.0               
             diag_4d(:,:,:,diag_pt%mfls_strat) =   &
                                             omega(:,:,:)+grav*Mc(:,:,:)
          end where
        end if
...
subroutine cloud_clear_xfer     &
             ( cloud_generator_on, tmp3, qa_mean, qa_mean_lst, a_clr,   &
                                                         a_cld, clr, cld)
       ...
       integer :: k,i,kdim,idim
        ...
        !compute cloud to clear transfer
          ...
          if (cloud_generator_on) then
             ...
             cld2clr = min(a_cld(i,k),  &
                              max(0.,tmp3(i,k)*tmp1+(1.-tmp3(i,k))*tmp2))
          end if

        !-------------------------------
        !compute clear to cloud transfer
          if (overlap .eq. 1)                                            &
             clr2cld = min(max(qa_mean(i,k)-qa_mean_lst(i,k),0.),     &
                                                               a_clr(i,k))
          if (overlap .eq. 2)                                            &
