...
!/                                                                      
     ...
     &  RHO,TEMP,VTEMP,COZEN,CDX,CHLB,OZB,BNAT,BICE,PHOTO,SOLARDATA,    &
     &  OZON,COSP,COSPHC,ANOY,AEROSOL,DT,MERIDS,CH_TEND,OZONE,          &
     &  O3_PROD,H2O_TEND,MYPE,ITIME)
! """" ----------------------------------------------------------------*
! VERSION NUMBER   : 1   DATE:  6/10/1989.                              
!   NOTES          : CODE REWRITTEN TO SOLVEEQUATIONS MORE DIRECTLY    
...
!                    TIME PERIOD IDT                                    
! METHOD           : USES THEEULER BACKWARD INTEGRATION SCHEME         
! INPUT/OUTPUT LIST:                                                    
...
!----------------------------------------------------------------------*
     ...
     &        ILX,IL_FIRST,JL,JLX,KL,ISTEPS,MERIDS,IZ,NC,KLX,IR,ITX,    &
     &        ITNO,ITIME_LEFT,MYPE,IX,IT,IM,IDX,ICYC,IDAYS,LEAPD,       &
     &        JMOD,ISC
...
!----------------------------------------------------------------------*
     ...
     &      CLWEIGHT(7),tracer1(merids,lats,levels),                    &
     &      dclydt(merids,lats,levels),dfdtau(merids,lats,8),           &
     &      cfc(merids,lats,9),cfct(9,2),dbrydt(merids,lats,levels),    &
...
!----------------------------------------------------------------------*
     ...
     &    1.0e-12*factor*dfdtau(il,jl,ic)*clweight(ic)*cfc(il,jl,ic)*   &
     &    agefact2
      enddo
...
!----------------------------------------------------------------------------
     ...
     &     H2O(MERIDS),H2SO4(MERIDS),COND(MERIDS,3),TICE(MERIDS),       &
     &     WH2SO4(MERIDS),AM(MERIDS),AW(MERIDS),ALIQ(MERIDS),           &
     &     RMEAN(MERIDS),ASAT(MERIDS),RNAT(MERIDS),RICE(MERIDS)
...
!
     ...
     &      RMEAN(MERIDS),GAMMA(MERIDS,NHET),RTT(MERIDS)
!
...
!------------------------------------------------------------------------
      ...
      REAL TEMP(MERIDS),PRESS(MERIDS),RHO(MERIDS),DYY(MERIDS,NCHEM),    &
     &     H2O(MERIDS),RATES(MERIDS,IHET+NHET-1),TICE(MERIDS),          &
...
!                                                                              
     ...
     &     SNATS(P_FIELD,LEVELS),SICES(P_FIELD,LEVELS),                 &
     &     F1(P_FIELD,LEVELS),F2(P_FIELD,LEVELS),PRESS(P_FIELD,LEVELS), &
     &     PNAT(P_FIELD),PICE(P_FIELD),PNAT2(P_FIELD),PICE2(P_FIELD),        &
     &     ANAT2(P_FIELD,LEVELS),AICE2(P_FIELD,LEVELS),                 &
     &     ANATMAX(P_FIELD),AICEMAX(P_FIELD),AHNO3(P_FIELD,LEVELS)
!                                                                              
...
!
     ...
     &     TIMECH4(36),CH4_A1B(36),CH4_A2(36),CH4_B1(36)
