...
*/ REVISED INTEGRATION SCHEME AFTER STEIL ET AL. 1998 14/6/99
...
*/                                                                      
      SUBROUTINE CHEMISTRY (IL_FIRST,JLX,KL,DY,H2O,DAGESQ,OZCOL,PRESS,
     2  RHO,TEMP,VTEMP,COZEN,CDX,CHLB,OZB,BNAT,BICE,PHOTO,SOLARDATA,
     3  OZON,COSP,COSPHC,ANOY,AEROSOL,DT,MERIDS,CH_TEND,OZONE,
     4  O3_PROD,H2O_TEND,MYPE,ITIME)
* """" ----------------------------------------------------------------*
...
* VERSION NUMBER   : 4   DATE: 09/02/1994 ET SEQ.                       
...
* INPUT/OUTPUT LIST:                                                    
*     JLX          = LAT.  NUMBER (NEEDED FOR ERROR DIAGNOSTICS)        
...
*----------------------------------------------------------------------*
      INTEGER ISTEP_CHEM,IC,IL,
     2        ILX,IL_FIRST,JL,JLX,KL,ISTEPS,MERIDS,IZ,NC,KLX,IR,ITX,
     3        ITNO,ITIME_LEFT,MYPE,IX,IT,IM,IDX,ICYC,IDAYS,LEAPD,
     4        JMOD,ISC
      REAL CH_TEND(MERIDS,NCHEM),DY(MERIDS,NCHEM),H2O(MERIDS),
     2     OZCOL(MERIDS),PRESS(MERIDS),RHO(MERIDS),TEMP(MERIDS),
     3     VTEMP(MERIDS),COZEN(MERIDS),CHLB(90,15),OZB(144,90),
     4     PHOTO(128,14,11,48),SOLARDATA(1801),OZON(11,48),COSP(14),
     5     COSPHC(48),ANOY(90,48),H2O_TEND(MERIDS),DAGESQ(MERIDS),
     6     BNAT(MERIDS),BICE(MERIDS),OZONE(MERIDS),O3_PROD(MERIDS),
     7     AEROSOL(MERIDS)
      INTEGER ITIME(6),IMON(12)
*----------------------------------------------------------------------*
* VARIABLES FOR THIS SUBROUTINE
*----------------------------------------------------------------------*
      REAL  DYY(MERIDS,NCHEM),DYZ(MERIDS,JIMPL),DQT(MERIDS,NCHEM),
     *      DPT(MERIDS,NCHEM),CHEMINC(MERIDS,NCHEM),
     *      RATES(MERIDS,IHET+NHET-1),FOTO(MERIDS,IRX4),   
     *      RAT(MERIDS,3),CLOX(MERIDS),BROX(MERIDS),
     *      ANOZ(MERIDS),DYZ1(MERIDS,JIMPL),DYY0(MERIDS,NCHEM),
     *      DYY1(MERIDS,NCHEM),H2O1(MERIDS), H2O0(MERIDS),DP1(MERIDS),
     *      DQ1(MERIDS),H2SO4(MERIDS),CDX,DVDT
      REAL  COND(MERIDS,3),TICE(MERIDS),WH2SO4(MERIDS),AM(MERIDS),
     *      AW(MERIDS),ALIQ(MERIDS),RMEAN(MERIDS),ASAT(MERIDS),
     *      RNAT(MERIDS),RICE(MERIDS),GAMMA(MERIDS,NHET),RTT(MERIDS)
...
*
     ...
     2      DELT,SUM,RATIO1,RATIO2,ANUM,ADEN,ANOX,DAX,DA1,DA2,DD1,DB1,
     3      DB2,DD2,DDNUM,DA3,DC2,DC3,DD3,R1,R2,DD13,VDDNUM,DAC2,DA12,
     4      DD12,CLX,TAU,RAT1,BX1,AX,BX,CX,XYZ,DQT1,DQT2,DQT3,SLAT,
     5      T0,T1,BRX,DIFF,VRHO,BX2,RATIO,DT,X1,X2,SOL,FRAC,SOLARTIME,
     6      SOLAR27,DX1,D11,D27,F11,F27,PI,O2,H2, minusT0
* ---------------------------------------------------------------------*
*                                                                       
*   THE ARRAYS DYY AND DYZ CONTAIN THE SPECIES AMOUNTS.  THE SPECIES    
*   DYY ARE CALCULATED EXPLICITLY, WHILE THE SPECIES DYZ ARE FOUND      
*   IMPLICITLY USING PHOTOCHEMICAL EQUILIBRIUM AND RELATED APPROACHES.  
*   THE VALUES IN DYZ ARE RECALCULATED AFRESH FOR EVERY CALL TO OAERO.  
...
*     ASSUME 12 EQUAL MONTHS PER YEAR OF 365.25 DAYS
...
*---                                                                    
* ---   SOLVE THREE WAY SIMULTANEOUS EQUATIONS                          
...
C
C FOTO(IL,27) IS TAKEN AS A PROXY FOR CH3O2 + HV --> H2CO + OH
C
...
*      XYZ = 1.E-15*RHO(IL)
*      IF (DYY1(IL,15).LT.XYZ) DYY1(IL,15) = XYZ 
*      XYZ = 1.E-9*RHO(IL)
*      IF (DYY1(IL,8).LT.XYZ) DYY1(IL,8) = XYZ 
!wfc++
!      enddo
  990 CONTINUE  
!wfc--
...
*
     ...
     2     (DPT(IL,8) - DQT(IL,8)*DYY(IL,8))*RAT(IL,1)*DELT/RHO(IL)
      ENDDO
...
*         
     ...
     2     agefact2,merids,lats,levels,itime)       
*----------------------------------------------------------------------*
...
*----------------------------------------------------------------------*
      IMPLICIT NONE
      INTEGER MDAY(12),itime(6) 
      INTEGER MERIDS,LATS,LEVELS,IANN,IMON,IDAY,IMON1,IT1,IT2,IC,KL,JL,
     2        IL,IY,IY1,IM,MX
      REAL  age(merids,lats,levels),dfdage(90,48,8),TROPC(151,9),
     2      CLWEIGHT(7),tracer1(merids,lats,levels),
     3      dclydt(merids,lats,levels),dfdtau(merids,lats,8),
     4      cfc(merids,lats,9),cfct(9,2),dbrydt(merids,lats,levels), 
     5      cly(merids,lats,levels),bry(merids,lats,levels) 
      REAL  YSTART,TFACT,AGEFACT1,AGEFACT2,DX1,DX2,TIME,DT1,DT2,SUM1,
     2      SUM2,FACTOR,YY,Y1,Y2,clytot,brytot
...
*----------------------------------------------------------------------*
     ...
     2    1.0e-12*factor*dfdtau(il,jl,ic)*clweight(ic)*cfc(il,jl,ic)*
     3    agefact2
      enddo
...
*-------------------------------------------------------------------------
      IMPLICIT NONE
      INTEGER  MERIDS,IHET,NHET,IL,IC
*
      REAL RHO(MERIDS),TEMP(MERIDS),VTEMP(MERIDS),PRESS(MERIDS),
     2         RATES(MERIDS,IHET+NHET-1)
*
      REAL TEMP3(MERIDS)
*
...
*     REACTION 7 NO2+NO3 + M --> N2O5 + M                               
        R0 = 2.0 E-30*RHO(IL)*TEMP3(IL)**(-4.4)
        R1 = 1.4 E-12*TEMP3(IL)**(-0.7)                                     
        RX = ALOG10(R0/R1)                                                
        RATES(IL,7) =(R0/(1.0 + R0/R1))*
     1                      (0.6**(1.0/(1.0 + (RX*RX)))) 
*     REACTION 8 N2O5+M --> NO3+NO2+M                                   
        RATES(IL,8) = RATES(IL,7)*3.333 E26*EXP(-10990.0*VTEMP(IL))    
...
*     REACTION 14 H+O2+M --> HO2+M : MULTIPLIED BY O2 NUMBER DENSITY    
        R0 = 5.7 E-32*RHO(IL)*TEMP3(IL)**(-1.6)
        R1 = 7.5 E-11*TEMP3(IL)**0.0                                     
        RX = ALOG10(R0/R1)                                                
...
*     REACTION 17 OH+NO2+M --> HNO3+M                                   
        R0 = 2.0 E-30*RHO(IL)*TEMP3(IL)**(-3.0)
        R1 = 2.5 E-11                                     
        RX = ALOG10(R0/R1)                                                
...
*     REACTION 23 CLO+NO2+M --> CLONO2+M                                
        R0 = 1.8 E-31*RHO(IL)*TEMP3(IL)**(-3.4)
        R1 = 1.5 E-11*TEMP3(IL)**(-1.9)                                     
        RX = ALOG10(R0/R1)                                                
...
*     REACTION 34 HO2+HO2 --> H2O2+O2                                   
     ...
     1                 1.7E-33*EXP(1000.0*VTEMP(IL))*RHO(IL)                
*     REACTION 35 OH+OH+M --> H2O2+M                                    
        R0 = 6.9 E-31*RHO(IL)*TEMP3(IL)**(-1.0)
        R1 = 2.6 E-11*TEMP3(IL)**0.0                                     
        RX = ALOG10(R0/R1)                                                
...
*     REACTION 37 CLO+CLO+M --> CL2O2 + M                               
        R0 = 1.6 E-32*RHO(IL)*TEMP3(IL)**(-4.5)
        R1 = 2.0 E-12*TEMP3(IL)**(-2.4)                                     
        RX = ALOG10(R0/R1)                                                
        RATES(IL,37) =(R0/(1.0 + R0/R1))*
     1                      (0.6**(1.0/(1.0 + (RX*RX)))) 
*     REACTION 38 CL2O2+M --> 2CLO + M                                  
        RATES(IL,38) = RATES(IL,37)*(7.87 E26*EXP(-8744.0*VTEMP(IL)))  
*     REACTION 39 BR + O3 --> BRO + O2                                  
        RATES(IL,39) = 1.7E-11*EXP(-800.0*VTEMP(IL))                        
*     REACTION 40 BRO + NO2 + M --> BRONO2 + M                          
        R0 = 5.2 E-31*RHO(IL)*TEMP3(IL)**(-3.2)
        R1 = 6.9 E-12*TEMP3(IL)**(-2.9)                                     
        RX = ALOG10(R0/R1)                                                
...
*     REACTION 49 HO2 + NO2 + M --> HNO4 + M                            
        R0 = 1.8 E-31*RHO(IL)*TEMP3(IL)**(-3.2)
        R1 = 4.7 E-12*TEMP3(IL)**(-1.4)                                     
        RX = ALOG10(R0/R1)                                                
...
*     REACTION 64 NO + O + M --> NO2 + M
        R0 = 9.0 E-31*RHO(IL)*TEMP3(IL)**(-1.5)
        R1 = 3.0 E-11                                     
        RX = ALOG10(R0/R1)                                                
        RATES(IL,64) =(R0/(1.0 + R0/R1))*
     1                      (0.6**(1.0/(1.0 + (RX*RX)))) 
*     REACTION 65 OH + NO + M --> HONO + M
        R0 = 7.0 E-31*RHO(IL)*TEMP3(IL)**(-2.6)
        R1 = 3.6 E-11*TEMP3(IL)**(-0.1)                                     
        RX = ALOG10(R0/R1)                                                
        RATES(IL,65) =(R0/(1.0 + R0/R1))*
     1                      (0.6**(1.0/(1.0 + (RX*RX)))) 
*     REACTION 66 N2O + OSD --> 2NO
*     Multiply by 0.75 for age of air correction
        RATES(IL,66) = 6.7 E-11*0.75
*     REACTION 67 N + NO --> N2 + O
        RATES(IL,67) = 2.1 E-11*EXP(100.0*VTEMP(IL))
*     REACTION 68 N + O2 --> NO + O
        RATES(IL,68) = 1.5 E-11*EXP(-3600.0*VTEMP(IL))
*     REACTION 69 CLO+OH --> --> HCL+O2                                
        RATES(IL,69) = 6.0 E-13*EXP(-230.0*VTEMP(IL))
*     REACTION 70 N2O + OSD --> N2 + O2
*     Multiply by 0.75 for age of air correction
        RATES(IL,70) = 4.9 E-11*0.75
*     REACTION 71 N + NO2 --> N2O + O
        RATES(IL,71) = 5.8 E-12*EXP(220.0*VTEMP(IL))
      ENDDO
      DO 20 IC=IHET,IHET+NHET-1                                         
        DO 20 IL=1,MERIDS                                               
          RATES(IL,IC) = 0.0                                            
   20 CONTINUE                                                        
      RETURN
...
*----------------------------------------------------------------------------
      IMPLICIT NONE
      INTEGER MERIDS,NCHEM,MYPE,IL
      REAL PRESS(MERIDS),RHO(MERIDS),TEMP(MERIDS),DYY(MERIDS,NCHEM),
     2     H2O(MERIDS),H2SO4(MERIDS),COND(MERIDS,3),TICE(MERIDS),
     3     WH2SO4(MERIDS),AM(MERIDS),AW(MERIDS),ALIQ(MERIDS),
     4     RMEAN(MERIDS),ASAT(MERIDS),RNAT(MERIDS),RICE(MERIDS)
      REAL DENS(MERIDS),VHET(MERIDS,4),WF(MERIDS,2),PH2O(MERIDS)
      REAL ANAT,AICE,AVGDR,RR,PI,ADROP,SIGMAL,TSAT,ABT,AMT,BMT,PX,C2,
     2     A1,A2,A3,A4,C3,C4,CLIMIT,P0H2O,Y1,Y2,T1,RMODE,RMODESAT,SANAT
*----------------------------------------------------------------------------
*
...
*-------------------------------------------------------------------------
     ...
     2       330918.55082/(TEMP(IL)**2) + 12725068.262/(TEMP(IL)**3))    
     ...
     2      - 93.317846778*AW(IL) + 273.88132245                         
        Y2 = -176.95814097*AW(IL)**(-0.36257048154) 
     2       - 90.469744201*AW(IL) + 267.45509988        
      ENDIF              
...
*                                                                       
     ...
     2            GAMMA,RTT,MERIDS,NCHEM,NHET,MYPE)
*-------------------------------------------------------------------------
*
...
*-------------------------------------------------------------------------
      IMPLICIT NONE
      INTEGER  MERIDS,NCHEM,NHET,MYPE,IL
*
      REAL  TEMP(MERIDS),PRESS(MERIDS),RHO(MERIDS),DYY(MERIDS,NCHEM),       
     2      H2O(MERIDS),WH2SO4(MERIDS),AW(MERIDS),AM(MERIDS),
     3      RMEAN(MERIDS),GAMMA(MERIDS,NHET),RTT(MERIDS)
*
...
*-------------------------------------------------------------------------
     ...
     2       CCLONO2,GAMMAB1,HHCL,AKHCL,Q1,RQ,A1,FCLONO2,GAMMARXN,
     3       GAMMABHCL,GAMMAS,FHCL,GAMMASP,GAMMABHCLP,GAMMAB,GCLONO2,
     4       SHOCL,HHOCL,AKHOCLCHOCL,FHOCL,WT,AK0,AK1,AK2,T0,HCLONO2,
     5       AMHCL,AKHOCL,CHOCL
*
...
*-------------------------------------------------------------------------
      DO 30 IL = 1,MERIDS
      AKH = 1.22 E12*EXP(-6200.0/TEMP2(IL))
      AKH2O = 1.95 E10*EXP(-2800.0/TEMP2(IL))     
      AKHYDR = AW(IL)*(AKH2O + AKH*AACID(IL))
      DIFF = 5.0 E-8*TEMP2(IL)/VISC(IL)
      SCLONO2 = 0.306 + 24.0/TEMP2(IL)
      HCLONO2 = 1.6 E-6*EXP(4710.0/TEMP2(IL) - SCLONO2*AMH2SO4(IL))
      CCLONO2 = 1474.*TEMP2(IL)**0.5
      GAMMAB1 = (4.0*HCLONO2*0.082*TEMP2(IL)/CCLONO2)*
     2          (DIFF*AKHYDR)**0.5
*
      HHCL = (0.094 - 0.61*XH2SO4(IL) + 1.2*XH2SO4(IL)**2)*
     2   EXP(-8.68 + (8515. - 10718.*XH2SO4(IL)**0.7)/TEMP2(IL))
      AMHCL = HHCL*DYY(IL,4)*PRESS(IL)/(RHO(IL)*101325.0)
      AKHCL = 7.9 E11*AACID(IL)*DIFF*AMHCL
...
*-------------------------------------------------------------------------
      ...
      HHOCL = 1.91 E-6*EXP(5862.4/TEMP2(IL) - SHOCL*AMH2SO4(IL))
      DIFF = 6.4 E-8 *TEMP2(IL)/VISC(IL) 
      AKHOCL = 1.25 E9*AACID(IL)*DIFF*AMHCL
      CHOCL = 2009.*RTT(IL)
      Q1 = (DIFF/AKHOCL)**0.5
      RQ = RMEAN(IL)/Q1
      A1 = RQ + 0.312*RQ**2
...
*                        
     ...
     1  RMEAN,RNAT,RICE,GAMMA,RTT,RATES,IHET,NHET,MERIDS,NCHEM,MYPE)
      IMPLICIT NONE
*------------------------------------------------------------------------
*
...
*------------------------------------------------------------------------
      INTEGER INN(9)
      INTEGER IHET, NHET, MERIDS, NCHEM, MYPE,IC,IL
      REAL TEMP(MERIDS),PRESS(MERIDS),RHO(MERIDS),DYY(MERIDS,NCHEM),
     1     H2O(MERIDS),RATES(MERIDS,IHET+NHET-1),TICE(MERIDS), 
     2     ALIQ(MERIDS),RMEAN(MERIDS),RNAT(MERIDS),RICE(MERIDS),
     3     GAMMA(MERIDS,NHET)
*
      REAL AMW(9),GNAT(9),GICE(9),RTT(MERIDS),
     1     CHEMC(MERIDS,NHET),CONST(NHET),
     2     G2NAT(MERIDS,NHET),DELT(MERIDS),SICE(MERIDS)
*
      REAL  ANAT,AICE,PI,GN,ANUM,ADEN
      DATA AMW/52.45, 108.00, 108.00, 97.45, 97.45, 96.91,
     2     52.45, 96.91, 141.91/               
*------------------------------------------------------------------------
*  AMW = MOLECULAR WEIGHT OF GAS PHASE SPECIES
*------------------------------------------------------------------------
      DATA PI/3.141592653589793/    
      DATA GNAT/0.1, 3.0 E-3, 4.0 E-4, 4.0 E-3, 0.2, 
     2          0.0, 0.0, 0.0, 0.0/
...
*-------------------------------------------------------------------------
      DO 60 IC = 1,NHET
        CONST(IC) = SQRT(8.0*8.3144 E7/(PI*AMW(IC)))
        DO 50 IL = 1,MERIDS
          IF(INN(IC).EQ.0) THEN
             CHEMC(IL,IC) = H2O(IL)
          ELSE
...
*                                                                       
     ...
     2        RATES(IL,IHET-1+IC) =  ANUM/ADEN 
  150   CONTINUE                                                        
*-------------------------------------------------------------------------
*                                                                       
...
*-------------------------------------------------------------------------
     ...
     1       RATES(IL,IHET+IC-1) =  RATES(IL,IHET+IC-1) +
     2       0.25*GAMMA(IL,IC)*CONST(IC)*RTT(IL)*ALIQ(IL)/CHEMC(IL,IC)      
 250  CONTINUE
*    
      RETURN                                                            
      END                                                               
...
*
      INTEGER MERIDS,IL                                                    
      REAL WF(MERIDS,2),T(MERIDS),DENS(MERIDS)                          
      REAL X(22),AMR(3)                                                 
      REAL W,WH,T2,V1,A1,A2,VS,VN,VMCAL
      DATA X/2.393284E-02,-4.359335E-05,7.961181E-08,0.0,-0.198716351,  
     1       1.39564574E-03,-2.020633E-06,0.51684706,-3.0539E-03,       
     2       4.505475E-06,-0.30119511,1.840408E-03,-2.7221253742E-06,   
     3      -0.11331674116,8.47763E-04,-1.22336185E-06,0.3455282,       
     4      -2.2111E-03,3.503768245E-06,-0.2315332,1.60074E-03,         
     5      -2.5827835E-06/                                             
...
*                                                                       
     ...
     2                 COSPHC,MERIDS,IRX,IRX4,FOTO,MYPE)
*---------------------------------------------------------------------- 
...
*---------------------------------------------------------------------- 
      ...
      REAL PHOTO(IRX4,14,11,48),COSP(14),OZON(11,48),                 
     *     VTEMP(MERIDS),OZONX(MERIDS),COSPHI(MERIDS),FOTO(MERIDS,IRX4), 
     *     X1(MERIDS),X2(MERIDS),Y1(MERIDS),Y2(MERIDS),Z1(MERIDS),      
     *     Z2(MERIDS),COSPHC(48),OZON1(MERIDS),OZON2(MERIDS),  
     *     COSP1(MERIDS),COSP2(MERIDS)  
      INTEGER IBD(4),IOZ(MERIDS),ICOZ(MERIDS)    
      LOGICAL LCOZ(MERIDS)
*
...
*
  ...
  140 CONTINUE                                                          
C                                                                       
C  INTERPOLATE LINEARLY IN COSPHI AND LINEARLY IN                       
C  OZONE AMOUNT FOR THE LOG OF THE PHOTODISSOCIATION RATE.              
C                                                                       
      DO 150 IL = 1,MERIDS                                              
      IOZ(IL) = 1                                                       
      OZON1(IL) = OZON(1,KL)                                            
  ...
  220 CONTINUE                                                          
     ...
     2        X2(IL)*PHOTO(IR,ICOZ(IL)+1,IOZ(IL),KL)                    
         P2 = PHOTO(IR,ICOZ(IL),IOZ(IL)+1,KL)*X1(IL) +                  
     2     X2(IL)*PHOTO(IR,ICOZ(IL),IOZ(IL),KL)                         
  ...
  240 CONTINUE                                                       
C                                                                       
C  PHOTODISSOCIATION RATES FOR TEMPERATURE DEPENDENT N2O5 CROSS-SECTION 
C  FIT A QUADRATIC TO THE VALUES AT 300K, 250K AND 200K                 
C                                                                       
      DO 250 IL = 1,MERIDS                                              
      IF(LCOZ(IL))  THEN  
...
C  COZEN, ALLOWING FOR LEAP YEARS AND THE ELLIPTICITY OF THE EARTH'S    
C  ORBIT.                                                               
C  ELONG IS THE LONGITUDE IN DEG. E. ,XLAT THE LATITUDE NORTH. GMT IS   
C  THE G.M.T. AND DAY IS THE DAY IN THE MONTH. TIMING INFO. IS GIVEN BY 
C  ITIME.                                                            
C                    
      IMPLICIT NONE
      INTEGER ITIME(6)
      REAL  DT,DRAD,DARG,DELTA,CDX,DGMT,DP,D1,D2,DF1,DTY,DTT,DR,DET,
     2      DAL1,DAL,CD1,DTD
*
...
*
   ...
   30 DTY = (DF1 - 693960.)/36525.                                      
      DTD = 100.*DTY                                                    
      DR = (2.7969668 E02 + 3.6000768925 E04*DTY + 3.03 E-4*DTY*DTY)*   
     2  DRAD                                                            
      DET = (-9.32701 E01  - 1.42 E-1*DTD)*SIN(DR) + (-4.3194497E02 +   
     2  3.3E-2*DTD)*COS(DR) + 5.965 E02*SIN(2.*DR) - 2.*COS(2.*DR)      
     3+ 4.2 E00*SIN(3.*DR) + 1.93E01*COS(3.*DR) - 1.28 E01*SIN(4.*DR)   
      DAL1 = DET*DRAD/240.0                                             
      DAL = DR - DAL1                                                   
C                                                                       
C  DELTA IS THE SOLAR DECLINATION AND COZEN IS THE COSINE OF THE SOLAR  
C  ZENITH ANGLE                                                         
C                                                                       
      DELTA = ATAN(4.336 E-1*SIN(DAL))                                  
      DARG = (DGMT - 12.0)*15.*DRAD + DAL1                              
      CD1 = DTD*DRAD*360.0                                              
      CDX = 1.00011 + 0.034221*COS(CD1) + 0.001280*SIN(CD1) +           
     2      0.000719*COS(2.*CD1) + 0.000077*SIN(2.*CD1)                 
      RETURN                                                            
      END                                                               
*
     ...
     2     PRESS,DT,MYPE)      
C      
C  CALCULATES SEDIMENTATION RATES OF TYPE I AND TYPE 2 PARTICLES               
C  AND VERTICALLY ADVECTS MODEL NAT AND ICE                                    
C                                                                              
      INTEGER P_FIELD, LEVELS
      REAL ANAT(P_FIELD,LEVELS),AICE(P_FIELD,LEVELS),
     1     ANOY(P_FIELD,LEVELS),AH2O(P_FIELD,LEVELS),
     2     SNATS(P_FIELD,LEVELS),SICES(P_FIELD,LEVELS),
     3     F1(P_FIELD,LEVELS),F2(P_FIELD,LEVELS),PRESS(P_FIELD,LEVELS),
     5     PNAT(P_FIELD),PICE(P_FIELD),PNAT2(P_FIELD),PICE2(P_FIELD),       
     6     ANAT2(P_FIELD,LEVELS),AICE2(P_FIELD,LEVELS),
     7     ANATMAX(P_FIELD),AICEMAX(P_FIELD),AHNO3(P_FIELD,LEVELS)
C                                                                              
C  DATA: V1 IS SEDIMENTATION VELOCITY (M/S)                                    
C  OF ICE PARTICLES V2 IS SEDIMENTATION VELOCITY OF         
C  NAT PARTICLES, ASSUMED RADII R1, R2.
C  AM1, AM2 ARE THE MOLECULAR WEIGHTS AND RHO1, RHO2 ARE THE DENSITIES 
c  OF THE PSCs IN G/CM3                                           
C                                                                              
      DATA  V1, V2/1.27 E-2, 1.39 E-4/
      DATA  R1, R2/7.0E-6, 0.5E-6/                                          
      DATA AM1,AM2/18.0,117.0/
      DATA RHO1,RHO2/0.928, 1.35/
C                                                                              
C  CALCULATE FRACTION OF NAT PARTICLES USED AS TYPE 2 CORES (F1)               
C  AND FRACTION OF NAT PARTICLES THAT REMAIN AS TYPE 1 CORES (F2)              
C  DETERMINE MAXIMUM NAT AND ICE TO APPLY LIMITERS TO ADVECTED AMOUNTS
C                                                                              
      ANATMAX(:) = 0.0
      AICEMAX(:) = 0.0
      ...
      DO 20 KL = 1,LEVELS 
      DO 10 JL = 1,P_FIELD 
      IF(AICE(JL,KL).LT.1.0 E-18) THEN 
         AICE(JL,KL) = 0.       
      ENDIF                                                                    
      IF(ANAT(JL,KL).LT.1.0 E-18)  THEN
         ANAT(JL,KL) = 0.
         F1(JL,KL) = 0.                                                        
         F2(JL,KL) = 0.                                                        
      ELSE                                                                     
   ...
   20 CONTINUE                                                                 
C                                                                              
C VERTICALLY ADVECT NAT AND ICE. NOTE THAT PART OF NAT IS ADVECTED             
C AT TYPE 2 RATE AND THE REMAINDER AT TYPE 1 RATE. CALCULATE DESCENT IN
C  1 TIMESTEP; USE APPROXIMATE VERTICAL DISPLACEMENT BETWEEN LAYERS
C                                                                              
      TEMP = 195.0
      PNAT(:) = 0.0
      PICE(:) = 0.0
   ...
   50 CONTINUE
C
C  set sedimented nat and ice to zero at top and bottom
C
      SNATS(:,1) = 0.0
      SICES(:,1) = 0.0 
      SNATS(:,LEVELS) = 0.0
...
C
C  APPLY LIMITERS TO NEW NAT AND ICE
C
...
C
  ...
  110 CONTINUE
C
C  ADJUST NOY AND H2O TENDENCY FIELDS
C
      DO KL = 1,LEVELS
      DO JL = 1,P_FIELD
...
!
     ...
     2     TIMECH4(36),CH4_A1B(36),CH4_A2(36),CH4_B1(36)
      INTEGER ITIME(6),IMON(12),ISC
      DATA  IMON/0,31,59,90,120,151,181,212,243,273,304,334/
      DATA TIMEN2O/
     2   1950.5, 1976.5, 1980.5, 1984.5, 1988.5, 1990.5,
     3   1992.5, 1994.5, 1996.5, 1998.5, 2000.5, 2010.5,
     4   2020.5, 2030.5, 2040.5, 2050.5, 2060.5, 2070.5,
     5   2080.5, 2090.5, 2100.5, 2500.5/
      DATA AN2O_A1B/
     2   278.0,  299.0,  301.0,  303.0,  305.0,  309.6,
     3   311.0,  312.0,  312.4,  314.0,  316.0,  324.0,
     4   331.0,  338.0,  344.0,  350.0,  356.0,  360.0,
     5   365.0,  368.0,  372.0,  372.0/
      DATA AN2O_A2/
     2   278.0,  299.0,  301.0,  303.0,  305.0,  309.6,
     3   311.0,  312.0,  312.4,  314.0,  316.0,  325.0,
     4   335.0,  347.0,  360.0,  373.0,  387.0,  401.0,
     5   416.0,  432.0,  447.0,  447.0/
      DATA AN2O_B1/
     2   278.0,  299.0,  301.0,  303.0,  305.0,  309.6,
     3   311.0,  312.0,  312.4,  314.0,  316.0,  324.0,
     4   333.0,  341.0,  349.0,  357.0,  363.0,  368.0,
     5   371.0,  374.0,  375.0,  375.0/
      DATA TIMECH4/
     2   1950.5, 1952.5, 1954.5, 1956.5, 1958.5, 1960.5,
     3   1962.5, 1964.5, 1966.5, 1968.5, 1970.5, 1972.5,
     4   1974.5, 1976.5, 1978.5, 1980.5, 1982.5, 1984.5,
     5   1986.5, 1988.5, 1990.5, 1992.5, 1994.5, 1996.5,
     6   1998.5, 2000.5, 2010.5, 2020.5, 2030.5, 2040.5,
     7   2050.5, 2060.5, 2070.5, 2080.5, 2090.5, 2100.5/
      DATA CH4_A1B/
     2   1147.5, 1163.8, 1182.1, 1202.4, 1224.2, 1247.5,
     3   1272.2, 1298.4, 1326.0, 1355.3, 1386.0, 1417.5,
     4   1449.3, 1481.5, 1514.0, 1547.1, 1580.9, 1614.2,
     5   1644.9, 1671.6, 1694.3, 1714.0, 1721.0, 1728.0,
     6   1745.0, 1760.0, 1871.0, 2026.0, 2202.0, 2337.0,
     7   2400.0, 2386.0, 2301.0, 2191.0, 2078.0, 1974.0/
      DATA CH4_A2/
     2   1147.5, 1163.8, 1182.1, 1202.4, 1224.2, 1247.5,
     3   1272.2, 1298.4, 1326.0, 1355.3, 1386.0, 1417.5,
     4   1449.3, 1481.5, 1514.0, 1547.1, 1580.9, 1614.2,
     5   1644.9, 1671.6, 1694.3, 1714.0, 1721.0, 1728.0,
     6   1745.0, 1760.0, 1861.0, 1997.0, 2163.0, 2357.0,
     7   2562.0, 2779.0, 3011.0, 3252.0, 3493.0, 3731.0/
      DATA CH4_B1/
     2   1147.5, 1163.8, 1182.1, 1202.4, 1224.2, 1247.5,
     3   1272.2, 1298.4, 1326.0, 1355.3, 1386.0, 1417.5,
     4   1449.3, 1481.5, 1514.0, 1547.1, 1580.9, 1614.2,
     5   1644.9, 1671.6, 1694.3, 1714.0, 1721.0, 1728.0,
     6   1745.0, 1760.0, 1827.0, 1891.0, 1927.0, 1919.0,
     7   1881.0, 1836.0, 1797.0, 1741.0, 1663.0, 1574.0/
!
