...
character(len=128), parameter :: version     = '$Id: moz.subs.F90,v 13.0.4.1 2006/11/20 20:24:10 wfc Exp $'
...
!-------------------------------------------------------
      ...
      rate(:,:,44) = 1.8e-11 * exp( 110. * itemp(:,:) )
      exp_fac(:,:) = exp( 70. * itemp(:,:) )
      rate(:,:,45) = 3.2e-11 * exp_fac(:,:)
      rate(:,:,186) = 3.0e-11 * exp_fac(:,:)
      exp_fac(:,:) = exp( 250. * itemp(:,:) )
      rate(:,:,49) = 3.5e-12 * exp_fac(:,:)
      rate(:,:,80) = 4.8e-11 * exp_fac(:,:)
      rate(:,:,50) = 3e-12 * exp( -1500. * itemp(:,:) )
      exp_fac(:,:) = exp( 180. * itemp(:,:) )
      rate(:,:,51) = 5.6e-12 * exp_fac(:,:)
      rate(:,:,86) = 4.2e-12 * exp_fac(:,:)
      rate(:,:,106) = 4.2e-12 * exp_fac(:,:)
      rate(:,:,112) = 2.2e-12 * exp_fac(:,:)
      rate(:,:,144) = 4.2e-12 * exp_fac(:,:)
      rate(:,:,149) = 4.2e-12 * exp_fac(:,:)
      exp_fac(:,:) = exp( -2450. * itemp(:,:) )
      rate(:,:,52) = 1.2e-13 * exp_fac(:,:)
      rate(:,:,211) = 8.5e-13 * exp_fac(:,:)
      exp_fac(:,:) = exp( 170. * itemp(:,:) )
      rate(:,:,53) = 2.3e-12 * exp_fac(:,:)
      rate(:,:,59) = 1.5e-11 * exp_fac(:,:)
      rate(:,:,192) = 1.8e-11 * exp_fac(:,:)
      rate(:,:,61) = 1.3e-12 * exp( 380. * itemp(:,:) )
      rate(:,:,63) = 2.45e-12 * exp( -1775. * itemp(:,:) )
      rate(:,:,65) = 3.e-12 * exp( 280. * itemp(:,:) )
      rate(:,:,66) = 5.e-13 * exp( -424. * itemp(:,:) )
      rate(:,:,67) = 1.9e-14 * exp( 706. * itemp(:,:) )
      rate(:,:,68) = 3.8e-13 * exp( 800. * itemp(:,:) )
      exp_fac(:,:) = exp( 200. * itemp(:,:) )
      rate(:,:,69) = 3.8e-12 * exp_fac(:,:)
      rate(:,:,75) = 3e-11 * exp_fac(:,:)
      rate(:,:,88) = 3.8e-12 * exp_fac(:,:)
      rate(:,:,104) = 3.8e-12 * exp_fac(:,:)
      rate(:,:,127) = 2.3e-11 * exp_fac(:,:)
      rate(:,:,147) = 3.8e-12 * exp_fac(:,:)
      rate(:,:,151) = 3.8e-12 * exp_fac(:,:)
      rate(:,:,169) = 3.8e-12 * exp_fac(:,:)
      rate(:,:,70) = 6.0e-13 * exp( -2058. * itemp(:,:) )
      rate(:,:,74) = 2.2e-11 * exp( 120. * itemp(:,:) )
      rate(:,:,76) = 1.5e-12 * exp( -880. * itemp(:,:) )
      rate(:,:,77) = 1e-14 * exp( -490. * itemp(:,:) )
      rate(:,:,79) = 2.9e-12 * exp( -160. * itemp(:,:) )
      rate(:,:,81) = 4.2e-12 * exp( -240. * itemp(:,:) )
      exp_fac(:,:) = exp( -2000. * itemp(:,:) )
      rate(:,:,82) = 5.5e-12 * exp_fac(:,:)
      rate(:,:,98) = 1.05e-14 * exp_fac(:,:)
      exp_fac(:,:) = exp( -1900. * itemp(:,:) )
      rate(:,:,84) = 6.5e-15 * exp_fac(:,:)
      rate(:,:,90) = 1.4e-12 * exp_fac(:,:)
      rate(:,:,85) = 4.6e-13 * exp( -1156. * itemp(:,:) )
      exp_fac(:,:) = exp( 700. * itemp(:,:) )
      rate(:,:,87) = 7.5e-13 * exp_fac(:,:)
      rate(:,:,101) = 7.5e-13 * exp_fac(:,:)
      rate(:,:,114) = 8.e-13 * exp_fac(:,:)
      rate(:,:,124) = 8.e-13 * exp_fac(:,:)
      rate(:,:,145) = 7.5e-13 * exp_fac(:,:)
      rate(:,:,150) = 7.5e-13 * exp_fac(:,:)
      rate(:,:,156) = 8.e-13 * exp_fac(:,:)
      rate(:,:,164) = 8.e-13 * exp_fac(:,:)
      exp_fac(:,:) = exp( 270. * itemp(:,:) )
      rate(:,:,89) = 5.6e-12 * exp_fac(:,:)
      rate(:,:,91) = 8.1e-12 * exp_fac(:,:)
      rate(:,:,194) = 7.4e-12 * exp_fac(:,:)
      exp_fac(:,:) = exp( 1040. * itemp(:,:) )
      rate(:,:,93) = 4.3e-13 * exp_fac(:,:)
      rate(:,:,130) = 4.30e-13 * exp_fac(:,:)
      exp_fac(:,:) = exp( 640. * itemp(:,:) )
      rate(:,:,94) = 1.3e-12 * exp_fac(:,:)
      rate(:,:,131) = 1.3e-12 * exp_fac(:,:)
      rate(:,:,166) = 1.3e-12 * exp_fac(:,:)
      rate(:,:,97) = 2.5e-12 * exp( 500. * itemp(:,:) )
      rate(:,:,99) = 8.7e-12 * exp( -1070. * itemp(:,:) )
      rate(:,:,100) = 2.6e-12 * exp( 365. * itemp(:,:) )
      rate(:,:,108) = 1.6e11 * exp( -4150. * itemp(:,:) )
      rate(:,:,109) = 1.2e-14 * exp( -2630. * itemp(:,:) )
      rate(:,:,110) = 2.54e-11 * exp( 410. * itemp(:,:) )
      rate(:,:,111) = 1.55e-11 * exp( -540. * itemp(:,:) )
      exp_fac(:,:) = exp( 400. * itemp(:,:) )
      rate(:,:,115) = 5.e-13 * exp_fac(:,:)
      rate(:,:,125) = 5.e-13 * exp_fac(:,:)
      rate(:,:,165) = 5.e-13 * exp_fac(:,:)
      rate(:,:,117) = 4.13e-12 * exp( 452. * itemp(:,:) )
      rate(:,:,118) = 7.52e-16 * exp( -1521. * itemp(:,:) )
      exp_fac(:,:) = exp( 175. * itemp(:,:) )
      rate(:,:,119) = 1.86e-11 * exp_fac(:,:)
      rate(:,:,161) = 1.86e-11 * exp_fac(:,:)
      rate(:,:,120) = 4.4e-15 * exp( -2500. * itemp(:,:) )
      exp_fac(:,:) = exp( 360. * itemp(:,:) )
      rate(:,:,121) = 2.7e-12 * exp_fac(:,:)
      rate(:,:,122) = 1.3e-13 * exp_fac(:,:)
      rate(:,:,128) = 5.3e-12 * exp_fac(:,:)
      rate(:,:,154) = 2.7e-12 * exp_fac(:,:)
      rate(:,:,162) = 2.7e-12 * exp_fac(:,:)
      exp_fac(:,:) = exp( 530. * itemp(:,:) )
      rate(:,:,132) = 4.6e-12 * exp_fac(:,:)
      rate(:,:,133) = 2.3e-12 * exp_fac(:,:)
      rate(:,:,136) = 1.2e-11 * exp( 444. * itemp(:,:) )
      rate(:,:,137) = 9.9e-15 * exp( -730. * itemp(:,:) )
      rate(:,:,138) = 5.6e-11 * exp( -650. * itemp(:,:) )
      rate(:,:,141) = 1.5e-11 * exp( -3600. * itemp(:,:) )
      rate(:,:,142) = 2.1e-11 * exp( 100. * itemp(:,:) )
      rate(:,:,143) = 1.0e-11 * exp( -660. * itemp(:,:) )
      rate(:,:,146) = 3.75e-13 * exp( -40. * itemp(:,:) )
      rate(:,:,153) = 3.03e-12 * exp( -446. * itemp(:,:) )
      rate(:,:,157) = 8.4e-13 * exp( 830. * itemp(:,:) )
      exp_fac(:,:) = exp( -1860. * itemp(:,:) )
      rate(:,:,158) = 1.4e-12 * exp_fac(:,:)
      rate(:,:,160) = 1.4e-12 * exp_fac(:,:)
      rate(:,:,167) = 1.90e-12 * exp( 190. * itemp(:,:) )
      exp_fac(:,:) = exp( -600. * itemp(:,:) )
      rate(:,:,170) = 6.7e-12 * exp_fac(:,:)
      rate(:,:,205) = 1.5e-11 * exp_fac(:,:)
      rate(:,:,171) = 7.e-12 * exp( -235. * itemp(:,:) )
      rate(:,:,180) = 1.9e-13 * exp( 520. * itemp(:,:) )
      ...
      rate(:,:,190) = 9.6e-12 * exp( -1360. * itemp(:,:) )
      ...
      rate(:,:,202) = 3.4e-12 * exp( 540. * itemp(:,:) )
      ...
      ko(:,:) = 8.e-27 * itemp(:,:)**3.5
      kinf(:,:) = 3.e-11
      call jpl( rate(1,1,83), m, .5, ko, kinf, plnplv )

      ko(:,:) = 8.5e-29 * itemp(:,:)**6.5
      kinf(:,:) = 1.1e-11 * itemp(:,:)
      call jpl( rate(1,1,92), m, .6, ko, kinf, plnplv )

      ko(:,:) = 1.e-28 * itemp(:,:)**.8
      kinf(:,:) = 8.8e-12
      call jpl( rate(1,1,105), m, .6, ko, kinf, plnplv )

      ko(:,:) = 8.e-27 * itemp(:,:)**3.5
      kinf(:,:) = 3.e-11
      call jpl( rate(1,1,172), m, .5, ko, kinf, plnplv )

      ko(:,:) = 3.e-31 * itemp(:,:)**3.3
      kinf(:,:) = 1.5e-12
      call jpl( rate(1,1,178), m, 0.6, ko, kinf, plnplv )

      ko(:,:) = 5.7e-32 * itemp(:,:)**1.6
      kinf(:,:) = 7.5e-11
