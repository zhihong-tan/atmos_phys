...
!-------------------------------------------------------
      ...
      rate(:,:,150) = 3.8e-12 * exp_fac(:,:)
      rate(:,:,154) = 3.8e-12 * exp_fac(:,:)
      rate(:,:,173) = 3.8e-12 * exp_fac(:,:)
      rate(:,:,210) = 5.5e-12 * exp_fac(:,:)
      ...
      rate(:,:,147) = 4.2e-12 * exp_fac(:,:)
      exp_fac(:,:) = exp( 700. * itemp(:,:) )
      rate(:,:,88) = 7.5e-13 * exp_fac(:,:)
      rate(:,:,102) = 7.5e-13 * exp_fac(:,:)
      rate(:,:,115) = 8.e-13 * exp_fac(:,:)
      rate(:,:,125) = 8.e-13 * exp_fac(:,:)
      rate(:,:,148) = 7.5e-13 * exp_fac(:,:)
      rate(:,:,153) = 8.6e-13 * exp_fac(:,:)
      rate(:,:,159) = 8.e-13 * exp_fac(:,:)
      rate(:,:,168) = 8.e-13 * exp_fac(:,:)
      exp_fac(:,:) = exp( 350. * itemp(:,:) )
      rate(:,:,90) = 4.63e-12 * exp_fac(:,:)
      rate(:,:,111) = 3.1e-11 * exp_fac(:,:)
      ...
      rate(:,:,99) = 1.0e-14 * exp( -1970. * itemp(:,:) )
      rate(:,:,100) = 7.66e-12 * exp( -1020. * itemp(:,:) )
      ...
      rate(:,:,157) = 2.7e-12 * exp_fac(:,:)
      rate(:,:,166) = 2.7e-12 * exp_fac(:,:)
      exp_fac(:,:) = exp( 640. * itemp(:,:) )
      rate(:,:,132) = 1.3e-12 * exp_fac(:,:)
      rate(:,:,170) = 1.3e-12 * exp_fac(:,:)
      exp_fac(:,:) = exp( 530. * itemp(:,:) )
      rate(:,:,133) = 4.6e-12 * exp_fac(:,:)
      rate(:,:,134) = 2.3e-12 * exp_fac(:,:)
      rate(:,:,183) = 1.87e-13 * exp_fac(:,:)
      rate(:,:,137) = 1.2e-11 * exp( 444. * itemp(:,:) )
      rate(:,:,138) = 9.9e-15 * exp( -730. * itemp(:,:) )
      rate(:,:,139) = 5.6e-11 * exp( -650. * itemp(:,:) )
      rate(:,:,144) = 1.5e-11 * exp( -3600. * itemp(:,:) )
      rate(:,:,145) = 2.1e-11 * exp( 100. * itemp(:,:) )
      rate(:,:,146) = 8.7e-12 * exp( -615. * itemp(:,:) )
      rate(:,:,149) = 3.75e-13 * exp( -40. * itemp(:,:) )
      rate(:,:,156) = 3.3e-12 * exp( -450. * itemp(:,:) )
      rate(:,:,160) = 8.4e-13 * exp( 830. * itemp(:,:) )
      exp_fac(:,:) = exp( -1860. * itemp(:,:) )
      rate(:,:,161) = 1.4e-12 * exp_fac(:,:)
      rate(:,:,164) = 1.4e-12 * exp_fac(:,:)
      rate(:,:,171) = 1.90e-12 * exp( 190. * itemp(:,:) )
      rate(:,:,174) = 2.9e-12 * exp( -345. * itemp(:,:) )
      rate(:,:,181) = 1.2e-11 * exp( -280. * itemp(:,:) )
      rate(:,:,185) = 1.7e-12 * exp( -710. * itemp(:,:) )
      rate(:,:,186) = 1.4e-10 * exp( -470. * itemp(:,:) )
      rate(:,:,188) = 2.3e-11 * exp( -200. * itemp(:,:) )
      rate(:,:,189) = 2.8e-11 * exp( 85. * itemp(:,:) )
      exp_fac(:,:) = exp( 290. * itemp(:,:) )
      rate(:,:,190) = 6.4e-12 * exp_fac(:,:)
      rate(:,:,211) = 4.1e-13 * exp_fac(:,:)
      exp_fac(:,:) = exp( -800. * itemp(:,:) )
      rate(:,:,192) = 2.9e-12 * exp_fac(:,:)
      rate(:,:,202) = 1.7e-11 * exp_fac(:,:)
      rate(:,:,209) = 1.7e-11 * exp_fac(:,:)
      rate(:,:,193) = 7.3e-12 * exp( -1280. * itemp(:,:) )
      rate(:,:,194) = 2.6e-12 * exp( -350. * itemp(:,:) )
      exp_fac(:,:) = exp( 220. * itemp(:,:) )
      rate(:,:,196) = 2.7e-12 * exp_fac(:,:)
      rate(:,:,216) = 5.8e-12 * exp_fac(:,:)
      rate(:,:,198) = 8.1e-11 * exp( -30. * itemp(:,:) )
      exp_fac(:,:) = exp( 260. * itemp(:,:) )
      rate(:,:,204) = 2.3e-12 * exp_fac(:,:)
      rate(:,:,206) = 8.8e-12 * exp_fac(:,:)
      rate(:,:,205) = 4.5e-12 * exp( 460. * itemp(:,:) )
      rate(:,:,207) = 1.2e-10 * exp( -430. * itemp(:,:) )
      rate(:,:,208) = 4.8e-12 * exp( -310. * itemp(:,:) )
      rate(:,:,212) = 6.0e-13 * exp( 230. * itemp(:,:) )
      rate(:,:,213) = 4.5e-14 * exp( -1260. * itemp(:,:) )
      ...
      ko(:,:) = 8.e-27 * itemp(:,:)**3.5
      kinf(:,:) = 3.e-11
      call jpl( rate(1,1,176), m, .5, ko, kinf, plnplv )

      ko(:,:) = 3.3e-31 * itemp(:,:)**4.3
      kinf(:,:) = 1.6e-12
      call jpl( rate(1,1,180), m, 0.6, ko, kinf, plnplv )

      ko(:,:) = 4.4e-32 * itemp(:,:)**1.3
      kinf(:,:) = 4.7e-11 * itemp(:,:)**0.2
      call jpl( rate(1,1,187), m, 0.6, ko, kinf, plnplv )

      ko(:,:) = 1.8e-31 * itemp(:,:)**3.4
      kinf(:,:) = 1.5e-11 * itemp(:,:)**1.9
      call jpl( rate(1,1,191), m, 0.6, ko, kinf, plnplv )

      ko(:,:) = 6.9e-31 * itemp(:,:)**1.0
      kinf(:,:) = 2.6e-11
      call jpl( rate(1,1,199), m, 0.6, ko, kinf, plnplv )
      ...
      call jpl( rate(1,1,203), m, 0.6, ko, kinf, plnplv )

      ko(:,:) = 9.0e-32 * itemp(:,:)**1.5
      kinf(:,:) = 3.0e-11
      call jpl( rate(1,1,215), m, 0.6, ko, kinf, plnplv )
