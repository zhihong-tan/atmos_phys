...
!-------------------------------------------------------
      ...
      exp_fac(:,:) = exp( -710. * itemp(:,:) )
      rate(:,:,207) = 1.7e-12 * exp_fac(:,:)
      rate(:,:,208) = 1.7e-12 * exp_fac(:,:)
      rate(:,:,209) = 1.7e-12 * exp_fac(:,:)
      rate(:,:,210) = 1.7e-12 * exp_fac(:,:)
      rate(:,:,211) = 1.7e-12 * exp_fac(:,:)
      rate(:,:,212) = 1.7e-12 * exp_fac(:,:)
      rate(:,:,213) = 1.4e-10 * exp( -470. * itemp(:,:) )
      rate(:,:,215) = 2.3e-11 * exp( -200. * itemp(:,:) )
      rate(:,:,216) = 2.8e-11 * exp( 85. * itemp(:,:) )
      exp_fac(:,:) = exp( 290. * itemp(:,:) )
      rate(:,:,217) = 6.4e-12 * exp_fac(:,:)
      rate(:,:,238) = 4.1e-13 * exp_fac(:,:)
      exp_fac(:,:) = exp( -800. * itemp(:,:) )
      rate(:,:,219) = 2.9e-12 * exp_fac(:,:)
      rate(:,:,229) = 1.7e-11 * exp_fac(:,:)
      rate(:,:,236) = 1.7e-11 * exp_fac(:,:)
      rate(:,:,220) = 7.3e-12 * exp( -1280. * itemp(:,:) )
      rate(:,:,221) = 2.6e-12 * exp( -350. * itemp(:,:) )
      exp_fac(:,:) = exp( 220. * itemp(:,:) )
      rate(:,:,223) = 2.7e-12 * exp_fac(:,:)
      rate(:,:,243) = 5.8e-12 * exp_fac(:,:)
      rate(:,:,225) = 8.1e-11 * exp( -30. * itemp(:,:) )
      exp_fac(:,:) = exp( 260. * itemp(:,:) )
      rate(:,:,231) = 2.3e-12 * exp_fac(:,:)
      rate(:,:,233) = 8.8e-12 * exp_fac(:,:)
      rate(:,:,232) = 4.5e-12 * exp( 460. * itemp(:,:) )
      rate(:,:,234) = 1.2e-10 * exp( -430. * itemp(:,:) )
      rate(:,:,235) = 4.8e-12 * exp( -310. * itemp(:,:) )
      rate(:,:,239) = 6.0e-13 * exp( 230. * itemp(:,:) )
      rate(:,:,240) = 4.5e-14 * exp( -1260. * itemp(:,:) )
      ...
      call jpl( rate(1,1,214), m, 0.6, ko, kinf, plnplv )

      ko(:,:) = 1.8e-31 * itemp(:,:)**3.4
      kinf(:,:) = 1.5e-11 * itemp(:,:)**1.9
      call jpl( rate(1,1,218), m, 0.6, ko, kinf, plnplv )

      ko(:,:) = 6.9e-31 * itemp(:,:)**1.0
      kinf(:,:) = 2.6e-11
      call jpl( rate(1,1,226), m, 0.6, ko, kinf, plnplv )

      ko(:,:) = 1.6e-32 * itemp(:,:)**4.5
      kinf(:,:) = 2.0e-12 * itemp(:,:)**2.4
      call jpl( rate(1,1,227), m, 0.6, ko, kinf, plnplv )
