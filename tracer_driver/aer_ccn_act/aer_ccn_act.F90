...
character(len=128) :: version = '$Id: aer_ccn_act.F90,v 14.0.2.1 2007/05/04 08:56:55 rsh Exp $'
...
if (lintegrate) then
  ...
  do i=ia,ib
    wtmp = tmp * x(i) + wm
    call aer_ccn_act( T, p, wtmp, totalmass, drop )
    sum1 = sum1 + w(i)*drop
    sum2 = sum2 + w(i)
  enddo
  ...
  call aer_ccn_act( T, p, wm, totalmass, drop )
