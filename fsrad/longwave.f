...
C-----------------------------------------------------------------------
C
C    Random band parameters for the longwave calcualtions using
C    10 cm-1 wide bands. The 15 um co2 complex is 2 bands,
C    560-670 and 670-800 cm-1. Ozone coefficients are in 3 bands,
C    670-800 (14.1 um), 990-1070 and 1070-1200 (9.6 um).
...
C    All bands are arranged in order of increasing wavenumbers.
C 
...
C
C     Data for ARNDM,BRNDM,AP,BP,ATP,BTP,AO3RND,BO3RND are obtained
C     by using the AFGL 1982 catalog. Continuum coefficients are from
C     Roberts (1976). This data was formerly in COMMON /BANDTA/.
...
C     ----------------------------------------------------------
C     The following data statements are band parameters obtained
C     using the 1982 AFGL catalog on the specified bands.
C     ----------------------------------------------------------
...
C-----------------------------------------------------------------------
C
C    Random band parameters for the longwave calculations using
C    comboned wide frequency bands between 160 and 1200 cm-1, 
C    as well as the 2270-2380 band for source calculations.
C
...
C     experimentation. This data was formerly in COMMON /BDCOMB/.
...
C-----------------------------------------------------------------------
C
C    Random band parameters for specific wide bands. At present,
C    the information consists of:  1) random model parameters for
C    the 15 um band, 560-800 cm-1; 2) the continuum coefficient for
C    the 800-990, 1070-1200 cm-1 band.
C
C    specifically:  
...
C     obtained by using the AFGL 1982 catalog. Continuum coefficients 
C     are from Roberts (1976). This data was formerly in
C     COMMON /BDWIDE/.
...
C-----------------------------------------------------------------------
C
C     Basic quantities computed in SUBROUTINE LWRAD and used in
C     the remaining longwave routines (formally COMMON /KDACOM/):
C
C       QH2O     =  H2O MASS MIXING RATIO,MULTIPLIED BY THE 
C                     DIFFUSIVITY FACTOR (DIFFCTR)
C       P        =  PRESSURE AT FLUX LEVELS OF MODEL
C       DELP2    =  PRESSURE DIFFERENCE BETWEEN FLUX LEVELS 
C       DELP     =  INVERSE OF DELP2
C       TTTT     =  TEMPERATURE ASSIGNED TO MODEL FLUX LEVELS 
C       VAR1     =  H2O OPTICAL PATH IN MODEL LAYERS (BETWEEN 
C                     FLUX LEVELS)
C       VAR2     =  PRESSURE-WEIGHTED H2O OPTICAL PATH IN MODEL LAYERS
C       VAR3     =  O3 OPTICAL PATH IN MODEL LAYERS 
C       VAR4     =  PRESSURE-WEIGHTED O3 OPTICAL PATH IN MODEL LAYERS 
C       CNTVAL   =  H2O CONTINUUM PATH IN MODEL LAYERS FOR THE
C                     800-990 AND 1070-1200 CM-1 COMBINED BAND
...
C-----------------------------------------------------------------------
C
C     Flux quantities computed by the radiation code, used for
C     diagnostic purposes (formally COMMON /RDFLUX/): 
C
C       FLX1E1     =  FLUX AT TOP FOR 0-160,1200-2200 CM-1 RANGE
C       GXCTS      =  FLUX AT TOP FOR 160-1200 CM-1 RANGE 
C       FCTSG      =  CTS FLUX AT GROUND. USED TO OBTAIN GXCTS
C                              BY BANDS.
...
C-----------------------------------------------------------------------
C
C     Planck function values used for the radiative calculations
C     (formally COMMON /SRCCOM/):
C
C       SORC     =  PLANCK FCTN, AT MODEL TEMPERATURES, FOR ALL BANDS 
C                     USED IN CTS CALCULATIONS
C       CSOUR1   =  PLANCK FCTN FOR 560-670 CM-1 BAND 
C       CSOUR2   =  PLANCK FCTN FOR 670-800 CM-1 BAND 
C       CSOUR    =  PLANCK FCTN FOR 560-800 CM-1 BANDS
C       OSOUR    =  PLANCK FCTN FOR 990-1070 CM-1 BAND
C       SS1      =  PLANCK FCTN FOR 800-990,1070-1200 CM-1 BANDS
...
C-----------------------------------------------------------------------
C
C     Quantities precomputed in subroutine TABLE for use in
C     the longwave radiation module (formally COMMON /TABCOM/):
C
...
C                   ELEMENTS OF AVEPHI,ETC.,IN FST88
C        KMAXVM  =  KMAXV(LMAX),USED FOR DO LOOP INDICES 
...
C     REAL  EM1   (28,180),EM1WDE(28,180),TABLE1(28,180),
C     COMMON /TABCOM/  EM1   (28,180),EM1WDE(28,180),TABLE1(28,180),
C    &                 TABLE2(28,180),TABLE3(28,180),EM3   (28,180)
      
C-----------------------------------------------------------------------
C
C     Transmission functions used for radiative computations, and
C     output heating rates and fluxes, except those needed out of
C     the radiative module (formally COMMON /TFCOM/): 
C
C       TO3      =  TRANSMISSION FCTN FOR THE 990-1070 CM-1 BAND
C                     O3(9.6 UM) + H2O CONTINUUM (NO LINES) 
C       CO21     =  TRANSMISSION FCTN FOR THE 560-800 CM-1 BAND 
C                     (AS 1 BAND). INCLUDES CO2 (IN LWRAD) AND
C                      H2O(L+C) AFTER MULTIPLICATION WITH "OVER"
C                      IN FST88 
C       EMISS    =  E2 EMISSIVITYY FCTN FOR H2O LINES (0-560,1200-2200
C                      CM-1). OBTAINED IN E1E288. 
C       EMISS2   =  TRANSMISSION FCTN FOR H2O CONTINUUM IN THE 800-990
C                      AND 1070-1200 CM-1 REGION, TAKEN AS 1 BAND 
C       AVEPHI   =  H2O OPTICAL PATHS BET. FLUX PRESSURES: INPUT TO 
C                      EMISSIVITY CALCULATIONS. 
C       TTEMP    =  TEMPERATURES USED AS INPUT FOR EMISSIVITY CALCS.
C       CTS      =  APPROX CTS HEATING RATES FOR 160-560 AND 800-990, 
C                      1070-1200 CM-1 RANGES
C       CTSO3    =  APPROX CTS HEATING RATES FOR 560-800,990-1070 CM-1
C                      RANGES 
C       EXCTS    =  EXACT CTS HEATING RATES FOR 160-1200 CM-1 RANGE 
C       EXCTSN   =  EXACT CTS HEATING RATES, BY BANDS 
C       E1FLX    =  E1 EMISSIVITY FCTN FOR H2O LINES (0-560,1200-CM-1)
C       CO2NBL   =  CO2 TRANS. FCTNS. (NOT PRESSURE-INTEGRATED) FOR 
C                      ADJACENT LEVELS,OVER THE 560-800 CM-1 RANGE. 
C       CO2SP1   =  CO2 TRANS. FCTNS. (NOT PRESSURE-INTEGRATED) BET.
C                      A FLUX LEVEL AND SPACE, FOR THE 560-670 CM-1 
C                      RANGE. USED FOR EXACT CTS CALCS. 
C       CO2SP2   =  SAME AS CO2SP1, BUT FOR THE 670-800 CM-1 RANGE. 
C       CO2SP    =  SAME AS CO2SP1, BUT FOR THE 560-800 CM-1 BAND.
C                      USED FOR APPROX CTS CALCS. 
C       TO3SPC   =  O3 OPTICAL DEPTHS BET. A LEVEL AND SPACE. USED FOR
C                      EXACT CTS CALCS. 
C       TOTVO2   =  H2O CONTINUUM OPTICAL PATHS BET. SPACE AND A
C                      LEVEL, USING THE CNT. COEFFICIENT FOR THE
C                      1-BAND 800-990,1070-1200 CM-1 BAND. USED FOR 
C                      CTS CALCS. 
...
C-----------------------------------------------------------------------
C                    LONG WAVE RADIATION CODE
C                    ------------------------
C   SUBROUTINE LWRAD (formally LWR88) COMPUTES TEMPERATURE-CORRECTED
C   CO2 TRANSMISSION FUNCTIONS AND ALSO COMPUTES THE PRESSURE GRID
C   AND LAYER OPTICAL PATHS.
C-----------------------------------------------------------------------
C                        INPUT PARAMETERS
C                        ----------------
C
...
C      PRES    =  PRESSURE (CGS UNITS) AT DATA LEVELS OF MODEL
...
C      RRVCO2  =  THE VOLUME MIXING RATIO OF CO2 (SCALAR)
C                 (used in NLTE, passed by argument list)
C
C      PSFC    =  Surface pressure, dimensioned by IMAX.
C      TSFC    =  Surface temperature, dimensioned by IMAX.
...
C                        -----------------
C
C      HEATRA     =  HEATING RATE AT DATA LEVELS (K/DAY) 
C      GRNFLX     =  NET LONGWAVE FLUX AT THE GROUND (CGS UNITS) 
C      TOPFLX     =  NET LONGWAVE FLUX AT THE TOP    (CGS UNITS)
C
C
C-----------------------------------------------------------------------
C         OTHER INPUTS (FROM COMMON BLOCKS):
C         ----------------------------------
...
C                         D I M E N S I O N
C    &  KTOP(IMAX,LP1),KBTM(IMAX,LP1),NCLDS(IMAX),EMCLD(IMAX,LP1),
C    &  TEMP(IMAX,LP1),PRES(IMAX,LP1),RH2O(IMAX,LMAX),QO3(IMAX,LMAX),
C    &  CAMT(IMAX,LP1),LSFC(IMAX),PSFC(IMAX)
C----------------OUTPUT ARGUMENTS---------------------------------------

      REAL,   INTENT(OUT), DIMENSION(:,:) :: HEATRA
      REAL,   INTENT(OUT), DIMENSION(:)   :: GRNFLX,TOPFLX

CCC   DIMENSION   HEATRA(IMAX,LMAX),GRNFLX(IMAX),TOPFLX(IMAX)
C-----------------------------------------------------------------------
C----------------LOCAL ARRAY STORAGE------------------------------------
      DIMENSION  PRESS(IMAX,LP1)

c     DIMENSION  CO2R(IMAX,LP1,LP1),DIFT(IMAX,LP1,LP1)
c     DIMENSION                     DIFT(IMAX,LP1,LP1)
...
C-----------------------------------------------------------------------
c     DIMENSION  DIFTD(IMAX,LP1,LP1) 
c     DIMENSION  DCO2DT(IMAX,LP1,LP1),D2CDT2(IMAX,LP1,LP1) 
      DIMENSION  TEXPSL(IMAX,LP1),TLSQU(IMAX,LP1)
      DIMENSION  VSUM4(IMAX,LMAX) 
      DIMENSION  DIFT1D(IMAX,LP1M) 
C-----------------------------------------------------------------------
C     EQUIVALENCE (DIFTD,CO2R)
C     EQUIVALENCE (DCO2DT,D2CDT2,CO2R)
C     EQUIVALENCE (VSUM3,TLSQU,TEXPSL)
C     EQUIVALENCE (VV,VSUM4)
C     EQUIVALENCE (DIFT1D,DIFT) 
C-----------------------------------------------------------------------
C----------- check presence of optional arguments ----------------------
     ...
     &                    'LSFC and PSFC must be used together.', FATAL)
      ENDIF
C-----------------------------------------------------------------------
C--- NOTE: convert input pressure (PRES) from MKS to CGS units
...
C****COMPUTE FLUX PRESSURES (P) AND DIFFERENCES (DELP2,DELP)
      ...
      ELSE
         DO I=1,IMAX 
            P(I,1)=0.0 
            P(I,LP1)=PRESS(I,LP1) 
C--- note: convert from MKS to CGS units
...
c2    DO KP=1,LP1 
c2    DO I=1,IMAX 
c        DIFTD(I,KP,K)=TSTDAV(I,KP)-TSTDAV(I,K)
c2       DIFT (I,KP,K)=TSTDAV(I,KP)-TSTDAV(I,K)
c2    ENDDO
c2    ENDDO
c2    ENDDO
...
c2    DO KP=1,LP1 
c2    DO I=1,IMAX 
c        DIFTD(I,KP,K)=1.0/(DIFTD(I,KP,K)+1.E-20) 
c2       DIFT (I,KP,K)=1.0/(DIFT (I,KP,K)+1.E-20) 
c2    ENDDO
c2    ENDDO
c2    ENDDO
...
c9    DO KP=1,LP1 
c9    DO I=1,IMAX 
c        DIFT(I,KP,K)=TDAV(I,KP)-TDAV(I,K) 
cd       DIFT(I,KP,K)=(TDAV(I,KP)-TDAV(I,K))*DIFT(i,kp,k) 
c9       DIFT(I,KP,K)=(TDAV(I,KP)-TDAV(I,K))*(1.0/(tstdav(i,kp)-
c9   &                 tstdav(i,k) + 1.E-20))
c9    ENDDO
c9    ENDDO
c9    ENDDO
...
c     DO KP=1,LP1 
c     DO I=1,IMAX 
c        DIFT(I,KP,K)=DIFT(I,KP,K)*DIFTD(I,KP,K) 
c     ENDDO
c     ENDDO
c     ENDDO
...
c9    DO K=2,LP1
c9    DO I=1,IMAX 
c9       DIFT(I,K,K)=VSUM4(I,K-1)
c9    ENDDO
c9    ENDDO

C****EVALUATE COEFFICIENTS FOR CO2 PRESSURE INTERPOLATION (A1,A2) 
C ---- NOTE: PRESS converted from MKS to CGS
...
C   STORAGE (AT A SLIGHT LOSS IN COMPUTATION TIME)
      ...
      DO KP=1,LP1 
      DO I=1,IMAX 
c        CO2R(I,KP,K)=A1(I)*CO251(KP,K)+A2(I)*CO258(KP,K)
...
c     DO KP=1,LP1 
c     DO I=1,IMAX 
c        DCO2DT(I,KP,K)=A1(I)*CDT51(KP,K)+A2(I)*CDT58(KP,K)
c     ENDDO
c     ENDDO
c     ENDDO
...
c     DO KP=1,LP1 
c     DO I=1,IMAX 
c        DCO2DT(I,KP,K)=1.E-2*DCO2DT(I,KP,K)
c     ENDDO
c     ENDDO
c     ENDDO
...
c        CO21(I,KP,K)=CO21(I,KP,K)+DIFT(I,KP,K)*DCO2DT(I,KP,K) 
c5       CO21(I,KP,K)=CO21(I,KP,K)+DIFT(I,KP,K)*(1.0E-2*(a1(I)*
c5   &                CDT51(KP,K)+A2(i)*CDT58(KP,K))) 
...
c     DO KP=1,LP1 
c     DO I=1,IMAX 
c        D2CDT2(I,KP,K)=A1(I)*C2D51(KP,K)+A2(I)*C2D58(KP,K)
c     ENDDO
c     ENDDO
c     ENDDO
...
c     DO KP=1,LP1 
c     DO I=1,IMAX 
c        D2CDT2(I,KP,K)=1.E-3*D2CDT2(I,KP,K)
c     ENDDO
c     ENDDO
c     ENDDO
...
c        CO21(I,KP,K)=CO21(I,KP,K)+0.50*
c                     DIFT(I,KP,K)* 
c    &                DIFT(I,KP,K)*D2CDT2(I,KP,K) 
c7       CO21(I,KP,K)=CO21(I,KP,K)+0.50*
c7   &                DIFT(I,KP,K)* 
c7   &                DIFT(I,KP,K)*(1.E-3*(A1(I)*C2D51(KP,K) + A2(I)*
c7   &                c2D58(KP,K))) 
...
c8   &                DIFT(I,KP,K)* 
c8   &                DIFT(I,KP,K)* 
c    &          (0.5*((Tdav(i,kp) - tdav(i,k))*(1.0/ (tstdav(i,kp) - 
c    &                         tstdav(i,k) + 1.E-20)) *
c    &          (Tdav(i,kp) - tdav(i,k))* (1.0/ (tstdav(i,kp) - 
c    &                        tstdav(i,k) + 1.E-20)) ) )*
     &         ((1.E-3*(A1(I)*C2D51(KP,K) + A2(I)*c2D58(KP,K)))) 
c    &              (1.E-3*(A1(I)*C2D51(KP,K) + A2(I)*c2D58(KP,K))) 
        else if ( (kp .eq. k)  .and. (k .ge. 2) ) then
         CO21(I,KP,K)=CO21(I,KP,K)+0.50*
     &        vsum4(i,k-1)*
     &        vsum4(i,k-1)*
...
C    INVARIANT WITH RESPECT TO K,KP,AND SO (I,1,K)=(I,K,1)) 
      ...
      DO K=1,LP1
      DO I=1,IMAX 
c6       CO2SP1(I,K)=CO2R1(I,K)+DIFT(I,K,1)*
c6   &               (DCO2D1(I,K)+0.50*DIFT(I,K,1)*D2CD21(I,K)) 
c6       CO2SP2(I,K)=CO2R2(I,K)+DIFT(I,K,1)*
c6   &               (DCO2D2(I,K)+0.50*DIFT(I,K,1)*D2CD22(I,K)) 
...
C--- WE AREN''T DOING NBL TFS ON THE 100 CM-1 BANDS .
      DO K=1,LMAX
      DO I=1,IMAX 
c7       CO2NBL(I,K)=CO2MR(I,K)+DIFT(I,K,K+1)*(CO2MD(I,K)+0.50* 
c7   &                      DIFT(I,K,K+1)*CO2M2D(I,K)) 
...
C***COMPUTE TEMP. COEFFICIENT BASED ON TTTT(K) (SEE REF.2) 
      DO K=1,LP1
      DO I=1,IMAX
...
C
C      LSFC    =  Vertical index of the lowest model level,
C                    dimensioned by IMAX. (For eta coordinate)
C
C-----------------------------------------------------------------------
C                        OUTPUT PARAMETERS
C                        -----------------
...
C        BCLDS                             RDBITS 
C        INDX2,KMAXV,SOURCE,DSRCE          TABCOM 
...
C                         D I M E N S I O N
C    &  KTOP(IMAX,LP1),KBTM(IMAX,LP1),NCLDS(IMAX),EMCLD(IMAX,LP1),
C    &  TEMP(IMAX,LP1),PRESS(IMAX,LP1),CAMT(IMAX,LP1),LSFC(IMAX)
C----------------OUTPUT ARGUMENTS---------------------------------------
...
C----------------OUTPUT ARGUMENTS---------------------------------------
C     DIMENSION   HEATRA(IMAX,LMAX),GRNFLX(IMAX),TOPFLX(IMAX)
C-----------------------------------------------------------------------
C----------------LOCAL ARRAY STORAGE------------------------------------
     ...
     &           FTOP(IMAX),FBOT(IMAX) 
C-----------------------------------------------------------------------
C  ------DIMENSION OF VARIABLES EQUIVALENCED TO THOSE ABOVE------
...
c1    DIMENSION OVER(IMAX,LP1,LP1)
C     DIMENSION OVER1D(IMAX,LP1M)
c1    DIMENSION EMISST(IMAX,LP1,LP1)
...
C     EQUIVALENCE (VTMP3,TVAL,VSUM1,EMXX,TC,HEATEM) 
C     EQUIVALENCE (VTMP2,VTMP21,AVPHO3,FAC1,AVEPHJ) 
C     EQUIVALENCE (VTMP1,AVMO3,FAC2,AVVO2,OVER,EMISST)
C     EQUIVALENCE (DSORC,TOTEVV,DELPR1,OSS,SUM,FLXNET)
C     EQUIVALENCE (TOTPHI,DELPR2,CSS,FLX,Z1)
C     EQUIVALENCE (TOTO3,ALPSQ1,DTC,FLXTHK) 
C     EQUIVALENCE (TPHIO3,ALPSQ2,SS2,CEVAL) 
C     EQUIVALENCE (AVEPHI,AVEP1D,AVEP1),(EMI21D,EMISS2) 
C     EQUIVALENCE (OVER1D,OVER),(TO31D,TO3),(EMIS1D,EMISS)
C     EQUIVALENCE (CO21D,CO21)
C     EQUIVALENCE (ALP,CSUB)
C-----------------------------------------------------------------------
...
C 
C     CALL NLTE (PRESS,RRVCO2)
C     DO 141 K=1,INLTE
C     DO 141 I=1,IMAX 
C     CSOUR1(I,K)=SORC(I,K,9) 
C     CSOUR2(I,K)=SORC(I,K,10)
C141  CONTINUE
C 
C-----------------------------------------------------------------------
...
C       2)PERFORM COMPUTATIONS ON THESE ARRAYS TO OBTAIN TO3,OVER,
C  AVEPHJ: UPPER TRIANGLE OF THE SYMMETRIC MATRICES IN 1-D FORM 
C       3) LOAD NUMBERS INTO THE UPPER TRIANGLE OF THE 2-D
C  MATRICES TO3,OVER,AVEPHI,USING THE CDC Q8MERG FUNCTION 
C       4) FILL UP THE LOWER TRIANGLE,USING THE CDC "SCATTER" FUNCTION
...
C        4
C 
C***IN ORDER TO USE A MINIMUM OF STORAGE SPACE, THE CALCULATIONS HAVE 
C   BEEN ORDERED AS FOLLOWS:  
C 
C     ---------------------------------------
C     STAGE 1...COMPUTE O3 TRANSMISSION FCTNS
C     ---------------------------------------
...
C---SOME OF THE CALCS. BELOW ARE "STRIP-MINED" SO THAT VECTOR CALCS.
C   ARE RESTRICTED TO A LENGTH OF 65535.  THIS IS A CYBER 205 LIMIT.
C   (THE COMPILER HANDLES STRIP-MINING  OF FORTRAN 77 CODE).
...
c        DO KP=K+1,LP1 
c        DO I=1,IMAX 
c           OVER(I,KP,K)=VTMP2(I,KMAXS-(K+1)+KP)
c        ENDDO
c        ENDDO
c     ENDDO
...
c     DO K=1,KMAXVM 
c     DO I=1,IMAX
c        OVER(I,INDX1(K),INDX2(K))=VTMP2(I,K)
c     ENDDO
c     ENDDO


C     ------------------------
C     STAGE 4...COMPUTE EMISS2
C     ------------------------
...
C   COMPUTE 1-BAND CONTINUUM TRANSMISSIVITIES (EMISS2)
...
C        THE FIRST COMPUTATION IS FOR THE 15 UM BAND,WITH THE H2O 
C     TRANS. FCTN (OVER) COMBINED WITH THE CO2 FCTN (CO21). 
C 
...
c     DO KP=1,LP1 
c     DO I=1,IMAX 
c        CO21(I,KP,K)=CO21(I,KP,K)*OVER(I,KP,K)
c     ENDDO
c     ENDDO
c     ENDDO
...
C       PERFORM NBL COMPUTATIONS FOR THE CO2 15 UM BAND 
...
C    ONE-BAND CONTINUUM BAND (EMISS2 AND TO3). THE SF2 FUNCTION IS
...
C---THE SF2 FUNCTION IN PREV. VERSIONS IS NOW EXPLICITLY EVALUATED
      ...
      DO K=2,LLM1 
      DO I=1,IMAX 
C        C(I,K)=CSUB(I,K)*(HM5+CSUB(I,K)*
C    &                               (0.166666-CSUB(I,K)*0.0416666))
...
C     3) SPECIAL VALUES AT THE SURFACE (EMISS(LMAX,LP1),EMISS(LP1,L),
...
C---THE CALCULATION OF TTEMP HAS BEEN MOVED TO E1E288 TO SAVE STORAGE 
...
c1    DO K=1,LP1
c1    DO I=1,IMAX 
c1       EMISST(I,1,K)=TC(I,1)*E1FLX(I,K)+SS1(I,1)*EMISS2(I,K,1)+
c1   &                 OSOUR(I,1)*TO3(I,K,1)+CSOUR(I,1)*CO2SP(I,K)
c1    ENDDO
c1    ENDDO
...
c1    DO KP=2,LP1 
c1    DO I=1,IMAX 
c1       EMISST(I,KP,K)=DTC(I,KP)*EMISS(I,KP,K)+
c1   &                  SS2(I,KP)*EMISS2(I,KP,K)+OSS(I,KP)*TO3(I,KP,K)+
c1   &                  CSS(I,KP)*CO21(I,KP,K) 
c1    ENDDO
c1    ENDDO
c1    ENDDO
...
c1    DO KP=1,LP1 
c1    DO I=1,IMAX 
c1       EMISST(I,KP,K)=EMISST(I,KP,K)*CLDFAC(I,KP,K)
c1    ENDDO
c1    ENDDO
c1    ENDDO


C---THE ORDER OF THE 911 LOOPS IS TO MINIMIZE "MEMORY CONFLICTS"
C   ON THE CYBER 205
...
cf    DO KP=1,LP1 
      ...
      DO K=1,LP1
      DO I=1,IMAX 
cf       FLX(I,K)=FLX(I,K)+EMISST(I,KP,K)
         FLX(I,K)=FLX(I,K)+                    
...
C***THE OUTER LOOP IS ON THE NO. OF CLOUDS( BEGINNING AT NC=2,AS NC=1 IS
C   A DUMMY LAYER)
...
C***OBTAIN THE LARGEST NO. OF CLOUDS IN THE LAT. ROW. IF THE CLOUD
C   INDEX (NC) IS GREATER THAT THIS NO.,TERMINATE THE CLOUD LOOP
...
C***FOR THE NC''TH CLOUD LEVEL,OBTAIN THE TOP AND BOTTOM INDEX,AND 
C   COUNT THE NO. OF THICK CLOUDS (DEFINED AS A NONZERO DIFFERENCE
C   BET. THE TOP AND BOTTOM INDICES). SKIP TO THE NEXT CLOUD INDEX
C   (INCREMENT NC) IF THIS NO. IS ZERO. 
...
C   ALL LEVELS. THIS QUANTITY WILL BE USED ONLY IN THE THICK CLOUD
C   LEVELS, BUT, FOR EFFICIENCY, IS COMPUTED FOR ALL LEVELS.
...
C     TABLE1,TABLE2,TABLE,EM1,EM1WDE   TABCOM 
C     AVEPHI                           TFCOM
...
C**** I/O ARGUMENTS
CCC   DIMENSION FXOE1(IMAX,LP1),DTE1(IMAX,LP1),G1(IMAX,LP1),
CCC  &          G2(IMAX,LMAX),G3(IMAX,LP1),G4(IMAX,LP1),G5(IMAX,LMAX) 
CCC   DIMENSION  TEMP(IMAX,LP1)
C-----------------------------------------------------------------------
...
C     DIMENSION TTMP1D(IMAX,LP1M) 

      CHARACTER(LEN=40) :: err_string
C-----------------------------------------------------------------------
      EQUIVALENCE (EM1V(1),EM1(1,1)),(EM1VW(1),EM1WDE(1,1)) 
C     EQUIVALENCE (TTMP1D,TTEMP)
C     EQUIVALENCE (IVAL,IT1),(FIVAL,FIT1),(DT,WW1)
C     EQUIVALENCE (FXO,WW2),(F2,DTOT2),(F3,DTOT3) 
...
C-----------------------------------------------------------------------
C---THE REST OF THE SUBROUTINE IS CARRIED OUT WITHIN A K-LOOP (121) 
C    PRIMARILY TO CONSERVE STORAGE. SOME LOSS OF SPEED OCCURS.
C-----------------------------------------------------------------------
                         DO 121 K=1,LP1
C-----------------------------------------------------------------------
C 
...
C...REPEAT FOR WIDE-BAND COMPUTATIONS OF G4,G5
...
C     DIMENSION  PRESS(IMAX,LP1),TEMP(IMAX,LP1)
C-----------------------------------------------------------------------
...
C-----------------------------------------------------------------------
C     EQUIVALENCE (CAPPHI,AJ,AI,AH,AG,F,PHITMP) 
C     EQUIVALENCE (CAPPSI,AJJ,AII,AHH,AGG,FF,PSITMP)
C-----------------------------------------------------------------------

C     COMPUTE TEMP. TEMPEDENCE OF LINE INTENSITIES (CAPPHI,CAPPSI)
...
C*************BEGIN LOOP ON FREQUENCY BANDS (N)*************************
...
C   OBTAIN WEIGHED OPTICAL PATH AND MEAN PRESSURE (TOPM,TOPPHI) 
...
C***COMPUTE CTS TRANSMISSION FCTNS FOR ALL BANDS (TT) 
...
C***ADD H2O CONTINUUM PATHS FOR FREQ. BANDS 5-14
...
C***ADD O3 PATHS FOR BAND 13:990-1070 BAND
...
C***ADD THE CO2 TRANSMISSIVITIES FOR BANDS 9 AND 10 (15 UM BAND)
...
C*** COMPUTE EXACT CTS HEATING RATES (EXCTS,EXCTSN) 
...
C    OBTAIN EXACT CTS FLUXES (GXCTS,FCTSG) AND APPROX CTS HEATING 
C     RATES (CTSO3) 
...
C-----------------------------------------------------------------------
C     SUBROUTINE NLTE IS THE PRESENT FORMULATION OF AN NLTE 
C     CALCULATION OF THE SOURCE FCTN IN THE 15 UM REGION (2 BANDS)
C-----------------------------------------------------------------------
C
C     RRVCO2 IS THE VOLUME MIXING RATIO OF CO2 (SCALAR)
C
C-----------------------------------------------------------------------
C
C           INPUTS                       (COMMON BLOCKS)
C         BANDLO,BANDHI                   BANDTA
C         DELP                            KDACOM
C         CO21                            TFCOM 
C         CSOUR1,CSOUR2                   SRCCOM
C           OUTPUTS:  
C         SORC                            SRCCOM
C-----------------------------------------------------------------------
...
C    &  CAMT(IMAX,LP1)
C-----------------------------------------------------------------------
c     DIMENSION        V1(IMAX,LP1,LP1),
      DIMENSION        
     &                 CAMTV(IMAX),V11D(IMAX,LP1) 
C-----------------------------------------------------------------------
      LOGICAL BCLDUD,BCLDSA,BCLDSB,BCLDSC
      DIMENSION BCLDUD(2*IMAX,LP1),BCLDSA(IMAX,LP1),
     &          BCLDSB(IMAX,LP1),BCLDSC(IMAX,LP1)
C-----------------------------------------------------------------------
C                      ***LOGICAL ARRAY DEFINITIONS***
C
C   BCLDUD  =  T  IF (AT GRID POINT I) :1) VERTICAL LEVEL K IS BELOW
C                 THE TOP OF CLOUD (NC);2) VERTICAL LEVEL K IS NOT
C                 BELOW THE BOTTOM OF CLOUD (NC). FOR EACH VERTICAL
C                 LEVEL K,THE FIRST (IMAX) PTS. IS FOR TEST (1);
C                 THE SECOND (IMAX) PTS. IS FOR TEST (2).
C   BCLDSA  =  T  IF (AT GRID POINT I)  EITHER VERTICAL LEVEL K OR
C                 VERTICAL LEVEL KP IS BELOW THE TOP OF CLOUD (NC).
C   BCLDSB  =  T  IF (AT GRID POINT I) EITHER VERTICAL LEVEL K OR
C                 VERTICAL LEVEL KP IS NOT BELOW THE BOTTOM OF
C                 CLOUD (NC).
C   BCLDSC  = T  IF (AT GRID POINT I) :1)VERTICAL LEVEL K IS ABOVE CLOUD
C                (NC) AND VERTICAL LEVEL KP IS BELOW CLOUD (NC), OR VICE
C                VERSA; 2) VERTICAL LEVELS K,KP ARE BOTH BELOW THE TOP
C                OF CLOUD (NC) AND ABOVE THE BOTTOM OF CLOUD (NC)
C                BELOW THE TOP OF CLOUD (NC).
C-----------------------------------------------------------------------

C *** Initialize CLDFAC ***
C *** Note: If the user wants to initialize this quantity
C ***       then it must be passed as an argument (in & out)
...
C         *** IF NO GRID PT HAS AN (NCTH) CLOUD, ***
C         *** THE CLOUD CALCULATION IS COMPLETE  ***
...
C***INITIALIZE THE CLOUD TF FOR THE (NCTH) CLOUD TO UNITY.
c     DO K=1,LP1
c     DO KP=1,LP1
c     DO I=1,IMAX
c        V1(I,KP,K)=1.0
...
C***DEFINE THE CLOUD TRANSMISSIVITY.  THE TRANSMISSIVITY IS TAKEN AS
C  (1-CLOUD EMISSIVITY) FOR THE FRACTION OF THE GRID BOX WITH THE NCTH
C  LAYER OF CLOUD, AND ONE OTHERWISE. ( NOTE THAT IF AT POINT I, THE
C  NCTH CLOUD DOES NOT EXIST, ITS AMOUNT IS TAKEN AS ZERO). AS A RESULT,
C  THE CLOUD TRANSMISSIVITY MAY BE DEFINED AS THOUGH THE EMISSIVITY
C  WERE UNITY AND THE CLOUD AMOUNT WERE MULTIPLIED BY THE EMISSIVITY.
...
C***THE 221 LOOP IS THE K-LOOP. WE USED SUCH A LOOP PRIMARILY TO REDUCE
C    SPACE***
...
C---COMBINE BCLDSA AND BCLDSB INTO BCLDSC
      DO KP=1,LP1
      DO I=1,IMAX
...
C***DEFINE TRANSMISSION FUNCTION FOR LEVELS WHERE LOGICAL ARRAY BCLDSC
C   INDICATES THAT A CLOUD IS PRESENT

c     DO KP=1,LP1
c     DO I=1,IMAX
c        IF (BCLDSC(I,KP)) THEN
c           V1(I,KP,K)=V11D(I,KP)
c        ENDIF
c     ENDDO
c     ENDDO
...
C***RANDOM OVERLAP OF CLOUD IS ASSUMED IS OBTAINING THE TOTAL CLOUD TF**
c     DO K=1,LP1
      DO KP=1,LP1
      DO I=1,IMAX
c        CLDFAC(I,KP,K)=CLDFAC(I,KP,K)*V1(I,KP,K)
      if (bcldsc(i,kp) ) then
...
C-----------------------------------------------------------------------
C   Allocates temporary space for longwave code
C-----------------------------------------------------------------------
...
C-----------------------------------------------------------------------
C-------------- TABCOM + Initialize tables for longwave ----------------
