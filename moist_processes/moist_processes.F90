...
!------------------ allocatable moist processes variables --------------
   ...
   real, allocatable, dimension(:,:,:) :: mc_full, mc_donner, m_cdet_donner, massflux
...
! allocate all of necessary temporary arrays - done once
      call moist_alloc_init(ix, jx, kx, nt)
...
!---------------------------------------------------------------------
      ...
      call moistproc_uw_conv(Time, is, js, dt, tin, qin, uin, vin, tracer,    &
                             ...
                             shallow_liquid, shallow_ice, shallow_droplet_number)
    endif  !(do_uw_conv)
...
!---------------------------------------------------------------------
    if (do_strat .and. do_limit_donner) then
      call moistproc_scale_donner(qin, delta_temp, delta_q, precip_returned,    &
...
!--------------------------------------------------------------------- 
    ...
    do k=1,kx
      rain_don(:,:) = rain_don(:,:) + liquid_precip(:,:,k)* pmass(:,:,k)/seconds_per_day
      snow_don(:,:) = snow_don(:,:) + frozen_precip(:,:,k)* pmass(:,:,k)/seconds_per_day
    end do
...
!--------------------------------------------------------------------
      tin = tin+delta_temp
      qin = qin+delta_q
      call moist_conv (tin, qin, pfull, phalf, coldT, ttnd_don, qtnd_don, &
...
!---------------------------------------------------------------------
        ...
        call moistproc_uw_conv(Time, is, js, dt, tin_orig, qin_orig, uin, vin, tracer_orig,    &
                               pfull, phalf, zfull, zhalf, omega, pblht,        &
                               ustar, bstar, qstar, land, coldT, Aerosol,       &
                               cush, cbmf, cmf, conv_calc_completed,            &
                               available_cf_for_uw, tdt, qdt, udt, vdt, rdt,    &
...
!-----------------------------------------------------------------------
   if (do_ras) then
     ...
     call moistproc_ras(Time, is, js, dt, coldT, tin, qin, uin, vin, tracer,&
...
!        mc_cmt = mc
!        det_cmt = det0
         call moistproc_cmt ( Time, is, js, tin, uin, vin, tracer, pfull, phalf, &
...
!---------------------------------------------------------------------
   if (do_uw_conv) then
     if (do_limit_uw) then
       call moistproc_scale_uw(dt, qin, tracer, tdt, qdt, udt, vdt, rdt,  &
                               ttnd_conv, qtnd_conv, lprec, fprec, precip,&
...
! uw_conv diags
   ...
   if (id_enth_uw_col > 0) then
     ...
     call column_diag(id_enth_uw_col, is, js, Time, ttnd_uw, CP_AIR, qltnd_uw, -HLV, qitnd_uw, -HLS, temp_2d)
   endif

   if (id_wat_uw_col > 0) then
     temp_2d = rain_uw + snow_uw
     call column_diag(id_wat_uw_col, is, js, Time, qtnd_uw, 1.0, qltnd_uw, 1.0, qitnd_uw, 1.0, temp_2d)
   endif
...
!@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
      ...
      call moistproc_strat_cloud(Time, is, ie, js, je, ktop, dt, tm, tin, qin, tracer,  &
      ...
      if ( n /= nsphum ) then
        if ( .not. do_strat .or. (n /= nql .and. n /= nqi .and. n /= nqa .and. n /= nqn) ) then
          ...
          call wet_deposition( n, t, pfull, phalf, zfull, zhalf, rain, snow,   &
                               qtnd_wet, cloud_wet, cloud_frac,rain3d, snow3d, &
...
!---------------------------------------------------------------------
    call sum_diag_integral_field ('prec', precip*SECONDS_PER_DAY, is, js)
