...
public    &
        ...
        ! from longwave_utilities_mod
          lw_output_type, lw_table_type, &
          lw_diagnostics_type, assignment(=), &
        ! from gas_tf
          get_control_gas_tf
...
logical,              intent(in) :: do_h2o, do_o3, do_ch4, do_n2o, do_co2, do_cfc
...
!  local variables:
      ...
      real, dimension (size(pflux,1), &
                       size(pflux,2), &
                           6+NBTRGE ) :: trans_b2d1, trans_b2d2
...
!----------------------------------------------------------------------
      if ( .not. associated (Lw_diagnostics%flx1e1)) then
        call sealw99_alloc (ix, jx, kx, do_totcld_forcing, Lw_diagnostics)
      else
...
! </SUBROUTINE>
!
subroutine sealw99_alloc (ix, jx, kx, do_totcld_forcing, Lw_diagnostics)
